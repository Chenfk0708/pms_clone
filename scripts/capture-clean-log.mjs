...
const taskId = 'fangtai--baojie-guanli--baojie-rizhi'
const targetUrl = 'https://minsubao.localhome.cn/cleanManage/cleanLog'
const cloneUrl = process.env.PMS_CLONE_URL ?? 'http://127.0.0.1:4173/cleanManage/cleanLog'
...
async function runInteraction(page, side, action) {
  ...
  if (action.option) {
    clickResult.optionResult = await clickByText(page, action.option)
...
function interactionActions() {
  return [
    { slug: 'operation-date', label: '开始日期' },
    { slug: 'room-picker', label: '请选择房间' },
    { slug: 'operator-picker', label: '请选择操作人' },
    { slug: 'search-action', label: '搜 索' },
    { slug: 'reset-action', label: '重 置' },
  ]
}
...
async function extractState(page) {
  return page.evaluate(() => {
    ...
    return {
      ...
      controls: Array.from(document.querySelectorAll(selectors.join(','))).slice(0, 160).map(readNode),
      tables: Array.from(document.querySelectorAll('table,.ant-table,[class*="table"],[class*="Table"]')).slice(0, 24).map(readNode),
...
async function summarizeTransientState(page) {
  return page.evaluate(() => {
    ...
    return {
      ...
      selectedText: visibleTexts('.is-selected,.is-active,.ant-select-selection-item,.ant-picker-cell-selected'),
    }
  })
}
