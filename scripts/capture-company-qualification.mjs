...
const TARGET_URL = 'https://minsubao.localhome.cn/InformationMaintenance/qualification'
const LOCAL_URL = process.env.PMS_LOCAL_URL ?? 'http://127.0.0.1:4173/InformationMaintenance/qualification'
      ...
      JSON.stringify(
        {
          ...
          url: page.url(),
          isLoginBlocked: isBlocked(page.url(), facts.bodyText),
          hasBusinessText: hasBusinessText(facts.bodyText),
          ...
          panels: facts.panels.slice(0, 30),
          interactions,
          artifacts: {
            screenshotPath,
            fullScreenshotPath,
...
async function runInteractionSweep(page) {
  const interactions = []
  const actions = [
    { slug: 'business-license-tab', labels: ['营业资质'] },
    { slug: 'legal-id-tab', labels: ['法人证件'] },
    { slug: 'company-info-tab', labels: ['企业信息'] },
    { slug: 'edit-company-info', labels: ['编 辑', '编辑', '修改'] },
    { slug: 'primary', labels: ['新增', '添加', '立即认证', '去认证', '完善资质', '去完善'] },
    { slug: 'upload', labels: ['上传', '点击上传', '重新上传', '营业执照'] },
    { slug: 'submit', labels: ['提交', '保存', '确定'] },
    { slug: 'cancel', labels: ['取消', '关闭'] },
  ]
...
async function extractPageFacts(page) {
  return page.evaluate(() => {
    ...
    const panels = Array.from(
      document.querySelectorAll('.ant-card,.ant-form,.ant-table,.ant-alert,.ant-upload,.ant-tabs,.content,.panel,section'),
    )
      .filter(visible)
      .map(readStyle)
      .slice(0, 100)
...
function hasBusinessText(bodyText) {
  return bodyText.includes('企业资质') || bodyText.includes('资质信息') || bodyText.includes('营业执照')
}
