...
const taskId = 'scrm--yingxiao-tuiguang--youhuiquan'
const targetUrl = 'https://minsubao.localhome.cn/mallManagement/couponMgt'
const cloneUrl = process.env.PMS_LOCAL_URL ?? 'http://127.0.0.1:4173/mallManagement/couponMgt'
...
async function captureSide(side, url, contextOptions) {
  ...
  try {
    ...
    await tryClickAndCapture(page, states, side, stamp, '派发任务', 'dispatch-task-tab')
    await tryClickAndCapture(page, states, side, stamp, '优惠券管理', 'coupon-list-tab')
    await tryClickAndCapture(page, states, side, stamp, '新建', 'new-coupon')
    await tryClickAndCapture(page, states, side, stamp, '新增优惠券', 'create-coupon')
    await tryClickAndCapture(page, states, side, stamp, '新建优惠券', 'create-coupon')
    await tryClickAndCapture(page, states, side, stamp, '导出明细', 'export')
...
async function extractFacts(page) {
  return page.evaluate(() => {
    ...
    const tableHeaders = Array.from(document.querySelectorAll('th,.ant-table-thead .ant-table-cell,.coupon-table__head > div'))
      .map((element) => (element.innerText || element.textContent || '').replace(/\s+/g, ' ').trim())
      .filter(Boolean)

    const tables = Array.from(document.querySelectorAll('table,.ant-table,.coupon-table'))
      .slice(0, 8)
      .map(describe)
    ...
    const keyElements = Array.from(
      document.querySelectorAll('.ant-form,.ant-card,.ant-table,.ant-empty,.ant-select,.page-content,.coupon-page'),
    )
      .slice(0, 80)
      .map(describe)
