...
const TARGET_URL = 'https://minsubao.localhome.cn/setting/customChannel'
const LOCAL_URL = process.env.PMS_LOCAL_URL ?? 'http://127.0.0.1:4173/setting/customChannel'
...
async function settleWithTimeout(promise, milliseconds = 5000) {
  await Promise.race([promise, new Promise((resolve) => setTimeout(resolve, milliseconds))]).catch(() => {})
}
...
async function applyState(page) {
  ...
  if (state === 'primary-action') {
    const collapsedChat = await clickFirstVisible(page, ['收起会话', '收 起', '收起'])
    if (collapsedChat) interactions.push({ action: 'collapse-chat-before-primary-action', clicked: collapsedChat })
    interactions.push({
      action: 'click-primary-action',
      clicked: await clickFirstVisible(page, ['新增渠道', '添加渠道', '新 增', '新增', '添加']),
    })
  }
  ...
  if (state === 'search') {
    const inputs = page.locator(
      'input[placeholder*="渠道"], input[placeholder*="搜索"], input[placeholder*="关键"], input[type="text"]',
    )
  ...
  return page.evaluate(
    ({ capturedInteractions, capturedComponentScreenshots }) => {
      ...
      const keyElements = visibleElements.filter((item) =>
        /自定义渠道|渠道|新增|编辑|删除|启用|停用|平台|编码|状态|排序|查询|重置|保存|取消|暂无数据|全部会话/.test(
          item.text,
        ),
      )
      ...
      return {
        ...
        hasBusinessText:
          bodyText.includes('自定义渠道') || bodyText.includes('渠道名称') || bodyText.includes('渠道编码'),
...
async function main() {
  ...
  try {
    ...
    const toolbarShot = await screenshotFirstVisible(
      page,
      ['main form', '.ant-form', '.toolbar-card', '.custom-channel-toolbar', '.ant-card:has(input)'],
      'component-toolbar',
    )
    if (toolbarShot) componentScreenshots.push(toolbarShot)
    const tableShot = await screenshotFirstVisible(
      page,
      ['.ant-table-wrapper', '.ant-table', 'table', '.custom-channel-table', '.custom-channel-list'],
      'component-table',
    )
    ...
    await settleWithTimeout(context.close())
  } finally {
    await settleWithTimeout(browser.close())
  }
}
