...
const taskId = 'scrm--zengzhang-huoke--piliang-jiahaoyou'
const targetUrl = 'https://minsubao.localhome.cn/customer/addBatch'
const cloneUrl = process.env.PMS_LOCAL_URL ?? 'http://127.0.0.1:4173/customer/addBatch'
...
async function applyState(page) {
  const interactions = []
  const labelsByState = {
    primary: ['立即开通', '添加客户', '添加好友', '立即添加', '立即授权', '前往授权', '批量加好友'],
    template: ['下载模板', '导入模板', '批量导入', '上传名单', '选择文件'],
    rule: ['加好友设置', '设置', '查看规则', '展开'],
    firstDropdown: [],
  }
...
async function extractFacts(page, interactions) {
  return page.evaluate((capturedInteractions) => {
    ...
    const tableHeaders = [...document.querySelectorAll('th,.ant-table-cell,[role="columnheader"],.add-batch-table__head > div')]
    ...
    const keyElements = visibleElements.filter((item) =>
      /批量|好友|客户|企微|企业微信|增长|获客|标签|员工|导入|授权|模板|微信/.test(item.text),
    )
    ...
    return {
      ...
      hasBusinessText:
        bodyText.includes('批量加好友') ||
        bodyText.includes('增长获客') ||
