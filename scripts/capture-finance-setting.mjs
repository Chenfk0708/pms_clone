...
const TASK_ID = 'shezhi--tongyong-shezhi--caiwu-shezhi'
const TARGET_URL = 'https://minsubao.localhome.cn/setting/finance'
const LOCAL_URL = process.env.PMS_LOCAL_URL ?? 'http://127.0.0.1:4173/setting/finance'
...
async function applyState(page) {
  ...
  if (state === 'primary-action') {
    interactions.push({
      ...
      clicked: await clickFirstVisible(page, ['保存', '编辑', '新增', '设置', '配置', '启用', '开通']),
    })
  }
...
async function extractFacts(page, interactions, componentScreenshots) {
  return page.evaluate(
    ({ capturedInteractions, capturedComponentScreenshots }) => {
      ...
      const visibleElements = [...document.querySelectorAll('body *')].filter(elementVisible).slice(0, 640).map(describe)
      const controls = [
      ...
      const keyElements = visibleElements.filter((item) =>
        /财务|收款|结算|支付|发票|开票|账单|金额|规则|配置|保存|重置|新增|编辑|启用|停用|暂无数据|全部会话/.test(
          item.text,
        ),
      )
      ...
      return {
        ...
        hasBusinessText:
          bodyText.includes('财务设置') ||
          bodyText.includes('收款') ||
          bodyText.includes('结算') ||
          bodyText.includes('发票') ||
...
async function main() {
  ...
  try {
    ...
    for (const [selectors, suffix] of [
      [['main form', '.ant-form', '.finance-setting-toolbar', '.ant-card:has(input)'], 'component-toolbar'],
      [['.ant-table-wrapper', '.ant-table', 'table', '.finance-setting-table', '.finance-setting-list'], 'component-table'],
