...
const taskId = 'shezhi--tongyong-shezhi--zhifu-fangshi-shezhi'
const targetUrl = 'https://minsubao.localhome.cn/setting/paymentSetting'
const cloneUrl = process.env.PMS_LOCAL_URL ?? 'http://127.0.0.1:4173/setting/paymentSetting'
...
async function applyState(page) {
  ...
  if (state === 'add-dialog') {
    const addButton = page.getByRole('button', { name: /新\s*增/ }).first()
    if ((await addButton.count().catch(() => 0)) > 0) {
      await addButton.click({ timeout: 4000 }).catch((error) => {
        interactions.push({ action: 'open-add-dialog', clicked: false, error: error.message.split('\n')[0] })
      })
      interactions.push({ action: 'open-add-dialog', clicked: true, url: page.url() })
      await page.waitForTimeout(900)
      return interactions
    }
  }
...
async function extractFacts(page, interactions) {
  return page.evaluate((capturedInteractions) => {
    ...
    const keyElements = visibleElements.filter((item) =>
      /支付|收款|付款|方式|微信|支付宝|现金|银行卡|通联|储值|启用|禁用|新增|编辑|删除|保存|取消|开关|展开|收起|全部/.test(
        item.text,
      ),
    )
    ...
    return {
      ...
      hasBusinessText:
        bodyText.includes('支付方式设置') ||
        bodyText.includes('支付方式') ||
        bodyText.includes('微信支付') ||
        bodyText.includes('支付宝') ||
        bodyText.includes('现金'),
