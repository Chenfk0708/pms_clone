...
const TASK_ID = 'shezhi--xinxi-weihu--tupian-shipin'
const TARGET_URL = 'https://minsubao.localhome.cn/setting/picturesAndVideos'
const LOCAL_URL = process.env.PMS_LOCAL_URL ?? 'http://127.0.0.1:4173/setting/picturesAndVideos'
...
async function applyState(page) {
  ...
  if (state === 'video-tab') {
    interactions.push({ action: 'click-video-tab', clicked: await clickFirstVisible(page, ['视频', '酒店视频']) })
  }

  if (state === 'image-tab') {
    interactions.push({ action: 'click-image-tab', clicked: await clickFirstVisible(page, ['图片', '酒店图片']) })
  }

  if (state === 'upload') {
    interactions.push({
      action: 'click-upload',
      clicked: await clickFirstVisible(page, ['上传', '新增', '添加', '上传图片', '上传视频']),
    })
  }

  if (state === 'new-folder') {
    interactions.push({
      action: 'click-new-folder',
      clicked: await clickFirstVisible(page, ['新建文件夹', '新建', '文件夹']),
    })
  }
  ...
  if (state === 'search') {
    const inputs = page.locator(
      'input[placeholder*="名称"], input[placeholder*="房型"], input[placeholder*="搜索"], input[placeholder*="关键"], input[type="text"]',
    )
    for (let index = 0; index < (await inputs.count().catch(() => 0)); index += 1) {
      const input = inputs.nth(index)
      if (!(await input.isVisible().catch(() => false))) continue
      await input.fill('大床')
      interactions.push({ action: 'fill-keyword', value: '大床', index })
      break
    }
    interactions.push({ action: 'click-query', clicked: await clickFirstVisible(page, ['查询', '搜索']) })
  }
...
async function extractFacts(page, interactions, componentScreenshots) {
  return page.evaluate(
    ({ capturedInteractions, capturedComponentScreenshots }) => {
      ...
      const visibleElements = [...document.querySelectorAll('body *')].filter(elementVisible).slice(0, 680).map(describe)
      const controls = [
      ...
      const keyElements = visibleElements.filter((item) =>
        /图片|视频|房型|门店|上传|新增|添加|排序|封面|展示|渠道|OTA|暂无数据|查询|重置|收起|全部会话/.test(item.text),
      )
      ...
      return {
        ...
        hasBusinessText: bodyText.includes('图片') || bodyText.includes('视频') || bodyText.includes('上传'),
...
async function main() {
  ...
  try {
    ...
    const toolbarShot = await screenshotFirstVisible(
      page,
      ['main form', '.ant-form', '.ant-card:has(input)', '.media-filter', '.pictures-videos-filter', '.setting-media-toolbar'],
      'component-toolbar',
    )
    if (toolbarShot) componentScreenshots.push(toolbarShot)
    const contentShot = await screenshotFirstVisible(
      page,
      [
        '.ant-table-wrapper',
        '.ant-table',
        'table',
        '.pictures-videos-table',
        '.media-grid',
    ...
    if (contentShot) componentScreenshots.push(contentShot)
      ...
      JSON.stringify(
        {
          ...
          videos: facts.videos.slice(0, 20),
