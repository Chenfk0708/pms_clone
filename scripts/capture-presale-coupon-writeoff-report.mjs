...
const TASK_ID = 'baobiao--yushouquan-shuju--yushouquan-hexiao-mingxi'
const TARGET_URL = 'https://minsubao.localhome.cn/statistics/preSaleCouponMall'
const LOCAL_URL = process.env.PMS_LOCAL_URL ?? 'http://127.0.0.1:4173/statistics/preSaleCouponMall'
...
async function applyState(page) {
  ...
  if (state === 'third-dropdown') {
    const select = page.locator('.ant-select-selector, [role="combobox"]').nth(2)
    if (await locatorVisible(select)) {
      await select.click({ timeout: 2500 })
      await page.waitForTimeout(900)
      interactions.push({ action: 'open-third-select', clicked: 'select-2' })
    }
  }

  if (state === 'query') {
    const inputs = page.locator(
      'input[placeholder*="订单"], input[placeholder*="券"], input[placeholder*="手机号"], input[placeholder*="搜索"], input[placeholder*="关键"], input[type="text"]',
    )
  ...
  return page.evaluate(
    ({ capturedInteractions, capturedComponentScreenshots }) => {
      ...
      const keyElements = visibleElements.filter((item) =>
        /预售券|核销|销售|券码|商品|订单|门店|日期|时间|查询|导出|暂无数据|合计|说明/.test(item.text),
      )
...
async function main() {
  ...
  try {
    ...
    const filterShot = await screenshotFirstVisible(
      page,
      ['main form', '.ant-form', '.ant-card:has(input)', '.coupon-report-query', '.report-filter-card'],
      'component-filters',
    )
    if (filterShot) componentScreenshots.push(filterShot)
    const tableShot = await screenshotFirstVisible(
      page,
      ['.ant-table-wrapper', '.ant-table', 'table', '.coupon-report-table', '.report-table'],
      'component-table',
    )
