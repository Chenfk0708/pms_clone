...
async function clickFirstVisible(page, labels) {
  for (const label of labels) {
    const candidates = [
      page.getByRole('button', { name: looseLabel(label) }).first(),
      page.getByRole('link', { name: looseLabel(label) }).first(),
      page.getByRole('tab', { name: looseLabel(label) }).first(),
...
async function applyState(page) {
  ...
  if (state === 'primary-action') {
    interactions.push({
      ...
      clicked: await clickFirstVisible(page, ['新增模板', '添加模板', '新增打印模板', '新 增', '新增', '添加', '编辑']),
    })
  }

  if (state === 'edit-first') {
    interactions.push({
      action: 'click-edit-first',
      clicked: await clickFirstVisible(page, ['编辑', '修 改', '修改', '设置', '打印设置']),
    })
  }

  if (state === 'first-dropdown') {
    const dropdown = page.locator('.ant-select-selector,[role="combobox"],[aria-haspopup="listbox"]').first()
    if (await locatorVisible(dropdown)) {
      await dropdown.click({ timeout: 2500 }).catch(() => null)
      await page.waitForTimeout(900)
      interactions.push({ action: 'open-first-dropdown', clicked: true })
    }
  }

  if (state === 'first-switch') {
    const control = page.locator('.ant-switch,[role="switch"],input[type="checkbox"]').first()
    if (await locatorVisible(control)) {
      await control.click({ timeout: 2500 }).catch(() => null)
      await page.waitForTimeout(900)
...
async function extractFacts(page, interactions, componentScreenshots) {
  return page.evaluate(
    ({ capturedInteractions, capturedComponentScreenshots }) => {
      ...
      const visibleElements = [...document.querySelectorAll('body *')].filter(isVisible).slice(0, 650).map(describe)
      const controls = [
      ...
      const tableHeaders = [...document.querySelectorAll('th,.ant-table-cell,[role="columnheader"]')]
        ...
        .filter(Boolean)
        .slice(0, 220)
      const tableRows = [...document.querySelectorAll('tbody tr,.ant-table-row,[role="row"],label,.ant-card')]
        .filter(isVisible)
        .map((element) => (element.innerText || element.textContent || '').replace(/\s+/g, ' ').trim())
        .filter(Boolean)
        .slice(0, 140)
      ...
      const keyElements = visibleElements.filter((item) =>
        /打印|模板|小票|预览|编辑|删除|新增|保存|取消|启用|停用|房单|账单|订单|渠道|全部会话/.test(item.text),
      )
      ...
      return {
        ...
        hasBusinessText: bodyText.includes('打印设置') || bodyText.includes('打印') || bodyText.includes('模板'),
...
async function main() {
  ...
  try {
    ...
    for (const shot of [
      await screenshotFirstVisible(page, ['main', '.page-content', '.ant-card', '.print-setting-page'], 'component-main'),
      await screenshotFirstVisible(page, ['.ant-table-wrapper', '.ant-table', 'table', '.print-setting-table'], 'component-table'),
      await screenshotFirstVisible(page, ['.ant-modal', '.ant-drawer', '[role="dialog"]'], 'component-dialog'),
      await screenshotFirstVisible(page, ['.ant-select-dropdown', '.ant-dropdown', '[role="listbox"]'], 'component-dropdown'),
    ]) {
