...
const taskId = 'ota--siyu--siyu-qudao'
const targetUrl = 'https://minsubao.localhome.cn/channels/private'
const localUrl = process.env.PMS_LOCAL_URL ?? 'http://127.0.0.1:4173/channels/private'
...
try {
  ...
  if (captureLocal) result.local = await capturePage('local', localUrl)
...
async function capturePage(side, url, contextOptions = {}) {
  ...
  try {
    ...
    page.on('response', async (response) => {
      ...
      const isRelevant =
        parsedUrl.hostname === 'hudson-prod.localhome.cn' ||
        parsedUrl.pathname.includes('/channels/private') ||
        parsedUrl.pathname.includes('/assets/index-')
      if (!isRelevant) return

      let responseSummary = null
      try {
        const contentType = response.headers()['content-type'] ?? ''
        if (contentType.includes('application/json')) responseSummary = summarizePayload(await response.json())
      } catch (error) {
    ...
    if (side === 'local') {
      await page.getByRole('article', { name: '品牌小程序' }).getByRole('button', { name: '订阅开通' }).click().catch(() => {})
    }

    const bodyText = await page.locator('body').innerText().catch(() => '')
    const isLoginBlocked = isBlocked(page.url(), bodyText)
    const screenshotPath = path.join(artifactDirs.screenshots, `contract-${side}-${stamp}.png`)
    const fullScreenshotPath = path.join(artifactDirs.screenshots, `contract-full-${side}-${stamp}.png`)
    const htmlPath = path.join(artifactDirs.dom, `contract-${side}-${stamp}.html`)
    const stylePath = path.join(artifactDirs.styles, `contract-${side}-${stamp}.json`)
    const networkPath = path.join(artifactDirs.network, `contract-${side}-${stamp}.json`)
      ...
      JSON.stringify(
        {
          ...
          url: page.url(),
          isLoginBlocked,
          forbiddenTermsFound: findForbiddenTerms(bodyText),
          requests: network,
        },
        null,
        2,
    ...
    return {
      ...
      forbiddenTermsFound: findForbiddenTerms(bodyText),
      artifacts: { screenshotPath, fullScreenshotPath, htmlPath, stylePath, networkPath },
    }
  } finally {
    await context.close()
  }
...
function redactSecrets(value) {
  ...
  return Object.fromEntries(
    Object.entries(value).map(([key, item]) => {
      if (/token|cookie|password|passwd|secret|authorization|mobile|phone|appKey|accid|sessionId|sessionSeq|pageContextId|mpQrUrl/i.test(key)) {
        return [key, '[REDACTED]']
      }
...
async function extractPageState(page) {
  return page.evaluate(() => ({
    ...
    controls: Array.from(document.querySelectorAll('button,input,select,[role="button"],[role="tab"],[role="option"],.ant-select'))
      .slice(0, 160)
      .map((node) => {
...
function findForbiddenTerms(text) {
  return ['mock 数据', '未接入', '阻塞', '后端未就绪', '后端接口未完成'].filter((term) => text.includes(term))
}

function isBlocked(url, bodyText) {
  return bodyText.includes('账号登录') || bodyText.includes('登录密码') || bodyText.includes('请按住滑块') || url.includes('/home') || url.includes('/login')
}
