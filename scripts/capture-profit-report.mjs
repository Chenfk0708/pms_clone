...
const TASK_ID = 'baobiao--tongji-baobiao--lirun-baobiao'
const TARGET_URL = 'https://minsubao.localhome.cn/statistics/profitReport'
const LOCAL_URL = process.env.PMS_LOCAL_URL ?? 'http://127.0.0.1:4173/statistics/profitReport'
...
async function applyState(page) {
  ...
  if (state === 'dropdown') {
    const select = page.locator('.ant-select-selector, [role="combobox"]').filter({ hasText: '请选择' }).first()
    if (await locatorVisible(select)) {
      await select.click({ timeout: 2500 }).catch(() => {})
      await page.waitForTimeout(900)
      interactions.push({ action: 'open-dropdown', clicked: 'first-select-placeholder' })
    } else {
      const clicked = await clickFirstVisible(page, ['房型', '渠道', '房型分组', '请选择'])
  ...
  if (state === 'query') {
    const inputs = page.locator('input[placeholder*="订单"], input[placeholder*="搜索"], input[placeholder*="房型"], input[placeholder*="渠道"]')
  ...
  return page.evaluate(
    ({ capturedInteractions, capturedComponentScreenshots }) => {
      ...
      const keyElements = visibleElements.filter((item) =>
        /利润报表|利润|净利润|收入|支出|成本|房费|佣金|门店|房型|渠道|暂无数据|导出|查询|统计/.test(item.text),
      )
      ...
      return {
        ...
        hasBusinessText: bodyText.includes('利润报表') || bodyText.includes('净利润') || bodyText.includes('利润'),
...
async function main() {
  ...
  try {
    ...
    const filterShot = await screenshotFirstVisible(
      page,
      ['main form', '.ant-form', '.ant-card:has(input)', '.toolbar-card', '.profit-filter-card', '.report-filter-card'],
      'component-filters',
    )
    if (filterShot) componentScreenshots.push(filterShot)
    const tableShot = await screenshotFirstVisible(
      page,
      ['.ant-table-wrapper', '.ant-table', 'table', '.profit-table', '.report-table'],
      'component-table',
    )
