...
const TASK_ID = 'baobiao--jiesuanbiao--pinpai-xiaochengxu-dingdan'
const TARGET_URL = 'https://minsubao.localhome.cn/statistics/statementOrder'
const LOCAL_URL = process.env.PMS_LOCAL_URL ?? 'http://127.0.0.1:4173/statistics/statementOrder'
  ...
  for (const label of labels) {
    ...
    for (const locator of candidates) {
      ...
      try {
        ...
        // Continue with the next visible candidate.
      }
    }
  }
  return null
...
async function applyState(page) {
  ...
  if (['expanded', 'date-picker', 'select-0', 'select-1', 'select-2', 'query'].includes(state)) {
    const clicked = await clickFirstVisible(page, ['展开'])
    interactions.push({ action: 'ensure-expanded', clicked })
  }

  if (state === 'collapsed') {
    const clicked = await clickFirstVisible(page, ['收起', '收 起'])
    interactions.push({ action: 'collapse', clicked })
  }

  if (state === 'date-picker') {
    const input = page
      .locator('input[placeholder*="开始"], input[placeholder*="日期"], input[placeholder*="时间"]')
      .first()
  ...
  if (state.startsWith('select-')) {
    ...
    const select = page.locator('.ant-select-selector,[role="combobox"]').nth(Number.isNaN(index) ? 0 : index)
    if (await locatorVisible(select)) {
      await select.click({ timeout: 2500 }).catch(async () => select.click({ force: true, timeout: 2500 }).catch(() => {}))
      await page.waitForTimeout(900)
  ...
  if (state === 'query') {
    const input = page.locator('input[placeholder*="订单"], input[placeholder*="手机号"], input[placeholder*="搜索"], input[type="text"]').last()
    if (await locatorVisible(input)) {
      await input.fill('205')
      interactions.push({ action: 'fill-keyword', value: '205' })
    }
  ...
  if (state === 'detail') {
    const clicked = await clickFirstVisible(page, ['查看详情', '详情', '查看'])
    interactions.push({ action: 'open-detail', clicked })
  }
  ...
  return page.evaluate(
    ({ capturedInteractions, capturedComponentScreenshots }) => {
      ...
      const keyElements = visibleElements.filter((item) =>
        /品牌小程序订单|结算表|结算状态|订单|房费|佣金|门店|渠道|暂无数据|导出|查询|合计|收起|展开/.test(item.text),
      )
      ...
      return {
        ...
        hasBusinessText:
          bodyText.includes('品牌小程序订单') ||
          bodyText.includes('结算表') ||
          bodyText.includes('结算状态') ||
...
async function main() {
  ...
  try {
    ...
    const filterShot = await screenshotFirstVisible(
      page,
      ['main form', '.ant-form', '.ant-card:has(input)', '.report-filter-card', '.statement-order-filter'],
      'component-filters',
    )
    if (filterShot) componentScreenshots.push(filterShot)
    const tableShot = await screenshotFirstVisible(
      page,
      ['.ant-table-wrapper', '.ant-table', 'table', '.statement-order-table'],
      'component-table',
    )
      ...
      JSON.stringify(
        {
          ...
          tableHeaders: facts.tableHeaders.slice(0, 130),
          tableRows: facts.tableRows.slice(0, 14),
