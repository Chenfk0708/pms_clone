...
const TASK_ID = 'baobiao--jiesuanbiao--juhe-fenxiao-dingdan'
const TARGET_URL = 'https://minsubao.localhome.cn/statistics/distributionOrder'
const LOCAL_URL = process.env.PMS_LOCAL_URL ?? 'http://127.0.0.1:4173/statistics/distributionOrder'
...
async function applyState(page) {
  const interactions = []
  const needsExpandedFilters = ['date-picker', 'first-dropdown', 'second-dropdown', 'query'].includes(state)

  if (needsExpandedFilters) {
  ...
  if (state === 'first-dropdown') {
    const select = page.locator('.ant-select-selector, [role="combobox"], .distribution-order-select, [aria-haspopup="listbox"]').nth(0)
  ...
  if (state === 'second-dropdown') {
    const selects = page.locator('.ant-select-selector, [role="combobox"], .distribution-order-select, [aria-haspopup="listbox"]')
    const select = selects.nth((await selects.count().catch(() => 0)) > 1 ? 1 : 0)
    if (await locatorVisible(select)) {
      await select.click({ timeout: 2500 })
      await page.waitForTimeout(900)
  ...
  if (state === 'query') {
    const inputs = page.locator(
      'input[placeholder*="订单"], input[placeholder*="手机号"], input[placeholder*="搜索"], input[placeholder*="关键"], input[type="text"]',
    )
  ...
  return page.evaluate(
    ({ capturedInteractions, capturedComponentScreenshots }) => {
      ...
      const keyElements = visibleElements.filter((item) =>
        /聚合分销订单|分销订单|结算表|结算状态|房费|佣金|门店|渠道|查询|导出|暂无数据|合计|订单/.test(item.text),
      )
  ...
  try {
    ...
    const filterShot = await screenshotFirstVisible(
      ...
      [
        ...
        '.ant-form',
        '[aria-label="聚合分销订单筛选"]',
        '.statistics-distribution-query',
        '.ant-card:has(input)',
        '.report-filter-card',
        '.distribution-report-filter',
      ],
      'component-filters',
    )
    ...
    const tableShot = await screenshotFirstVisible(
      page,
      ['.ant-table-wrapper', '.ant-table', 'table', '.distribution-report-table', '.report-table'],
      'component-table',
    )
