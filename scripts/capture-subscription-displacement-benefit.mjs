...
const TARGET_URL = 'https://minsubao.localhome.cn/version/displacementBenefit'
const LOCAL_URL = process.env.PMS_LOCAL_URL ?? 'http://127.0.0.1:4173/version/displacementBenefit'
...
async function extractPageFacts(page) {
  return page.evaluate(() => {
    ...
    const businessElements = visibleElements.filter((item) =>
      /置换|权益|订阅|应用|套餐|版本|开通|购买|支付|明细|到期|启用|停用|查询|重置|暂无数据/.test(
        item.text,
      ),
    )
...
async function runInteractionSweep(page) {
  ...
  const applyResult = await clickFirstMatching(page, [/申请尾房置换/])
  interactions.push({ name: 'apply-tail-room-displacement', result: applyResult })
  await page.screenshot({ path: fileFor(artifactRoots.screenshots, 'after-apply-tail-room-displacement', 'png') })
  ...
  if ((await startDate.count()) > 0) {
    await startDate.click({ timeout: 2500 }).catch(() => {})
    await page.waitForTimeout(900)
    interactions.push({ name: 'open-start-date-picker', result: { clicked: true } })
    await page.screenshot({ path: fileFor(artifactRoots.screenshots, 'after-start-date-picker', 'png') })
  } else {
    interactions.push({ name: 'open-start-date-picker', result: { clicked: false } })
  }

  const collapseResult = await clickFirstMatching(page, [/收 起/])
  interactions.push({ name: 'collapse-chat-dock', result: collapseResult })
  await page.screenshot({ path: fileFor(artifactRoots.screenshots, 'after-collapse-chat-dock', 'png') })

  const queryResetGroups = [[/查询/, /搜 索/], [/重置/, /重 置/]]
  for (const patterns of queryResetGroups) {
