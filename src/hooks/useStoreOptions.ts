...
type UseStoreOptionsInput = {
  fallbackOptions?: StoreOption[]
  includeAll?: boolean
  enabled?: boolean
}

export function useStoreOptions(input: UseStoreOptionsInput = {}) {
  ...
  const [storeOptions, setStoreOptions] = useState<StoreOption[]>(fallbackOptions)
...
function createStoreOptionsKey(options: StoreOption[]) {
  return options.map((option) => `${option.id}:${option.label}`).join('|')
}

function normalizeFallbackOptions(options: StoreOption[] | undefined, includeAll: boolean) {
  const normalized = options?.length ? options : includeAll ? [{ id: 'all', label: '全部门店' }] : []
  if (!includeAll || normalized.some((option) => option.id === 'all')) return normalized
