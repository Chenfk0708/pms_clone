...
export function NotificationCenterPage() {
    ...
    const categoryUnreadCounts = useMemo(() => provider === 'api'
        ...
        : CATEGORIES.reduce((result, category) => {
            result[category.key] = currentItems.filter((item) => item.category === category.key && item.status === 'unread').length;
            return result;
        }, { order: 0, alert: 0, activity: 0 }), [apiUnreadCount, currentItems, provider, selectedCategory]);
    ...
    function handleMarkSingleRead(item) {
        if (item.status === 'read')
            return;
        if (provider === 'api') {
        ...
        setItems((current) => current.map((candidate) => candidate.id === item.id ? { ...candidate, status: 'read', highlight: false } : candidate));
    }
    return (_jsxs("div", { className: "notification-center-page", children: [_jsx("pre", { hidden: true, "data-testid": "notification-center-service-contract", "data-provider": contract.provider, "data-endpoint": contract.endpoint, children: JSON.stringify(contract) }), _jsxs("div", { className: "notification-center-page__layout", children: [_jsx("aside", { className: "notification-center-page__sidebar", "aria-label": "\u901A\u77E5\u5206\u7C7B", children: CATEGORIES.map((category) => {
...
function resolveNotificationCenterProvider() {
    const configured = readRuntimeConfig('pms.notificationCenter.provider') ||
        readRuntimeConfig('pmsNotificationCenterProvider') ||
        import.meta.env.VITE_NOTIFICATION_CENTER_PROVIDER ||
        import.meta.env.VITE_PMS_NOTIFICATION_CENTER_PROVIDER;
    return configured === 'api' || configured === 'real' ? 'api' : 'mock';
}
function createSystemMessagePageRequest(category, tab, pageNum) {
    return {
...
async function fetchSystemMessagePage(request, category, signal) {
    ...
    return {
        ...
        items: (payload.data.list ?? []).map((item) => ({
...
async function fetchSystemMessageUnreadCount(category, signal) {
...
async function markAllApiMessagesRead(category) {
...
async function markApiMessageRead(category, messageId) {
