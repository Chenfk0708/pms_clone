...
const CATEGORY_DATA: Record<NotificationCategoryKey, NotificationCategoryData> = {
  order: {
    totalCount: 2154,
    items: [
      {
...
export function NotificationCenterPage() {
  ...
  const pageNumbers =
    totalPages <= 5 ? Array.from({ length: totalPages }, (_, index) => index + 1) : [1, 2, 3, 4, 5]

  function handleCategoryChange(category: NotificationCategoryKey) {
    setSelectedCategory(category)
    setSelectedTab('all')
    setPage(1)
  }

  function handleTabChange(tab: NotificationTabKey) {
    setSelectedTab(tab)
    setPage(1)
  }
  ...
  return (
    <div className="notification-center-page">
      <div className="notification-center-page__layout">
          ...
          {CATEGORIES.map((category) => {
            ...
            return (
              <button
                key={category.key}
                type="button"
                className={`notification-center-page__category${selectedCategory === category.key ? ' is-active' : ''}`}
                aria-pressed={selectedCategory === category.key}
                aria-label={badge > 0 ? `${category.label} ${badge}` : category.label}
        ...
        <section className="notification-center-page__panel" aria-label="通知中心">
          <header className="notification-center-page__toolbar">
            <div className="notification-center-page__tabs" role="tablist" aria-label="通知筛选">
              <NotificationTab
                label="全部"
                ariaLabel="全部"
              ...
              <NotificationTab
                label="未读"
                badge={unreadCount}
                ariaLabel={unreadCount > 0 ? `未读 ${unreadCount}` : '未读'}
            ...
            <div className="notification-center-page__toolbar-actions">
              <button
                type="button"
                className="notification-center-page__primary-action"
              ...
              <button
                type="button"
                className="notification-center-page__icon-action"
                aria-label="通知设置"
                onClick={() => navigate('/setting/wechatPushSetting')}
              >
                <svg viewBox="0 0 24 24" aria-hidden="true">
                  <path d="M12 3.5a1 1 0 0 1 .96.71l.44 1.41a6.96 6.96 0 0 1 1.5.61l1.34-.71a1 1 0 0 1 1.16.17l1.43 1.43a1 1 0 0 1 .18 1.16l-.72 1.34c.28.47.49.97.62 1.5l1.4.44a1 1 0 0 1 .71.96v2.02a1 1 0 0 1-.71.96l-1.4.44c-.13.53-.34 1.03-.62 1.5l.72 1.34a1 1 0 0 1-.18 1.16l-1.43 1.43a1 1 0 0 1-1.16.18l-1.34-.72c-.47.28-.97.49-1.5.62l-.44 1.4a1 1 0 0 1-.96.71H10a1 1 0 0 1-.96-.71l-.44-1.4a6.96 6.96 0 0 1-1.5-.62l-1.34.72a1 1 0 0 1-1.16-.18L3.17 18.7a1 1 0 0 1-.17-1.16l.71-1.34a6.96 6.96 0 0 1-.61-1.5l-1.41-.44A1 1 0 0 1 1 13.3v-2.02a1 1 0 0 1 .71-.96l1.41-.44c.12-.53.33-1.03.61-1.5l-.71-1.34a1 1 0 0 1 .17-1.16l1.43-1.43a1 1 0 0 1 1.16-.17l1.34.71c.47-.28.97-.49 1.5-.61l.44-1.41A1 1 0 0 1 10 3.5h2Zm-1 5.25a3.25 3.25 0 1 0 0 6.5 3.25 3.25 0 0 0 0-6.5Z" />
                </svg>
              </button>
            </div>
          </header>
          ...
          <div className="notification-center-page__table" role="table" aria-label="通知列表">
            <div className="notification-center-page__thead" role="row">
              ...
              <div role="columnheader">内容</div>
              <div role="columnheader">时间</div>
            </div>

            <div className="notification-center-page__tbody">
              {pageItems.length > 0 ? (
                pageItems.map((item) => (
                  <div className="notification-center-page__row" role="row" key={item.id}>
                    ...
                    <div className="notification-center-page__time-cell" role="cell">
                      {item.time}
                    </div>
                  </div>
                ))
              ) : (
                <div className="notification-center-page__empty">该分类消息均已读</div>
              )}
            </div>
          </div>

          <footer className="notification-center-page__pagination" aria-label="分页">
            <span>{`第 ${pageStart}-${pageEnd} 条/总共 ${totalCount} 条`}</span>
            <div className="notification-center-page__pagination-controls">
              <button
                type="button"
                className="notification-center-page__page-arrow"
                aria-label="上一页"
                disabled={page === 1}
              ...
              {pageNumbers.map((pageNumber) => (
                <button
                  key={pageNumber}
                  type="button"
                  className={`notification-center-page__page-number${page === pageNumber ? ' is-active' : ''}`}
              ...
              {totalPages > 5 ? <span className="notification-center-page__ellipsis">…</span> : null}
              {totalPages > 5 ? (
                <button
                  type="button"
                  className="notification-center-page__page-number"
              ...
              <button type="button" className="notification-center-page__page-size" aria-label="20 条/页">
                20 条/页
              </button>
            </div>
          </footer>
...
function NotificationTab({
  ...
  return (
    <button
      type="button"
      role="tab"
      className={`notification-center-page__tab${active ? ' is-active' : ''}`}
