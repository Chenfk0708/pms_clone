...
import { fetchWorkspaceDashboard, fetchWorkspaceLists, fetchWorkspaceAnalysis, resolveWorkspaceCampId, } from '../services/workspace';
...
function TrafficGroup({ title, items, mutedFrom }) {
    return (_jsxs("div", { className: "workspace-traffic-group", children: [_jsx("h3", { children: title }), _jsx("div", { children: items.map((item, index) => (_jsx("span", { className: index >= mutedFrom ? 'is-muted' : '', children: item }, `${item}-${index}`))) })] }));
}
