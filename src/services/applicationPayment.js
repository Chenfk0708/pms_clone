...
export async function fetchApplicationPaymentDetail(request, providerName = getApplicationPaymentProviderName(), signal) {
...
async function fetchMockApplicationPaymentDashboard(filters, signal) {
...
function adaptApplicationPaymentDashboard(envelope, filters, provider) {
    if (envelope.code !== 0) {
...
async function fetchApiApplicationPaymentDashboard(filters, signal) {
    const [resource, paymentTypes, stores, roomCategories, paymentWays] = await Promise.all([
        postHudson(APPLICATION_PAYMENT_RESOURCE_PATH, { campId: filters.campId }, signal),
        postHudson(APPLICATION_PAYMENT_TYPES_PATH, { campId: filters.campId }, signal),
        postHudson(APPLICATION_PAYMENT_STORE_PATH, { campId: filters.campId, pageSize: 999, pageNum: 1, channelId: 0, isAvailability: '1' }, signal),
        postHudson(APPLICATION_PAYMENT_ROOM_CATEGORY_PATH, {
            campId: filters.campId,
            pageSize: 999,
            pageNum: 1,
            roomCategoryName: '',
        ...
        postHudson(APPLICATION_PAYMENT_WAYS_PATH, { campId: filters.campId }, signal),
    ]);
...
async function fetchMockApplicationPaymentDetail(request, signal) {
...
function adaptApplicationPaymentDetail(envelope, request, provider) {
    if (envelope.code !== 0) {
...
async function fetchApiApplicationPaymentDetail(request, signal) {
    const [paymentGroups, weiRoomCategories, rooms] = await Promise.all([
        postHudson(APPLICATION_PAYMENT_TYPES_V2_PATH, { campId: request.campId, bizTypes: [3], isEnable: 1 }, signal),
        postHudson(APPLICATION_PAYMENT_WEI_ROOM_CATEGORY_PATH, {
            campId: detailBuyCampId,
            buyCampId: request.campId,
            roomCategoryTypes: [1],
            goodsTypes: [7],
        }, signal),
        postHudson(APPLICATION_PAYMENT_ROOMS_PATH, {
            campId: request.campId,
...
function createDetailView(definition, request, provider) {
    return {
        provider,
        request,
        product: {
...
function resolveDetailProductId(searchParams, routeState) {
    ...
    if (app === 'globalRadar')
        return 'global-radar';
    if (app === 'douyin')
        return 'douyin-direct';
    if (routeState?.product === 'scrm')
        return 'scrm';
    return 'smart-clean';
}
...
function toCategory(value) {
    if (value === 'channel' || value === 'feature')
        return value;
    return 'all';
}
