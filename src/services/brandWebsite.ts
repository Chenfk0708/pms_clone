export type BrandWebsiteProviderName = 'mock' | 'api'
export type BrandWebsiteMockMode = 'success' | 'empty' | 'error'

export interface BrandWebsiteQuery {
  campId?: string
  businessDate?: string
  keyword?: string
}

export interface BrandWebsiteStore {
  id: string
  name: string
}

export interface BrandWebsiteMetric {
  id: string
  label: string
  value: string
  unit: string
...
export interface BrandWebsiteTemplate {
  id: string
  name: string
  scene: string
...
export interface BrandWebsiteCoupon {
  id: string
  name: string
...
export interface BrandWebsiteTodo {
  id: string
  title: string
  owner: string
  dueText: string
...
export interface BrandWebsiteData {
  camp: BrandWebsiteStore
  stores: BrandWebsiteStore[]
  businessDate: string
  metrics: BrandWebsiteMetric[]
  templates: BrandWebsiteTemplate[]
  coupons: BrandWebsiteCoupon[]
  todos: BrandWebsiteTodo[]
  routeTargets: Array<{ label: string; path: string }>
  pageConfig: {
    storeName: string
    heroTitle: string
    primaryColor: string
    bottomNavigation: Array<{ id: string; label: string }>
    floatingButtonEnabled: boolean
    popupEnabled: boolean
  }
}

export interface BrandWebsiteResponse<T> {
  code: number
  message: string
  data: T
  traceId: string
...
export interface BrandWebsiteViewModel extends BrandWebsiteData {
  contract: {
    provider: BrandWebsiteProviderName
    scenario: BrandWebsiteMockMode
    traceId: string
    request: {
      path: string
      method: 'POST'
      body: Required<BrandWebsiteQuery>
    }
  }
}
...
const templates: BrandWebsiteTemplate[] = [
  {
    id: 'camping',
    name: '露营地主题模板',
...
const baseData: BrandWebsiteData = {
  camp: stores[0],
  stores,
...
export function loadBrandWebsiteData(query: BrandWebsiteQuery = {}): BrandWebsiteViewModel {
...
export function createMockBrandWebsiteResponse(
  query: Required<BrandWebsiteQuery>,
  scenario: BrandWebsiteMockMode,
): BrandWebsiteResponse<BrandWebsiteData | null> {
  if (scenario === 'error') {
    return {
      code: 50029,
      message: '品牌官网数据加载失败',
...
function adaptBrandWebsiteResponse(
  response: BrandWebsiteResponse<BrandWebsiteData | null>,
  query: Required<BrandWebsiteQuery>,
  provider: BrandWebsiteProviderName,
  scenario: BrandWebsiteMockMode,
): BrandWebsiteViewModel {
...
function filterCoupons(coupons: BrandWebsiteCoupon[], keyword: string) {
  if (!keyword) return coupons
  return coupons.filter((item) => item.name.includes(keyword))
}

function normalizeQuery(query: BrandWebsiteQuery): Required<BrandWebsiteQuery> {
  return {
