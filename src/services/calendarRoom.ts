...
import { fetchEnabledChannelCatalog, type ChannelCatalogItem, type CustomChannelProviderName } from './customChannel'
...
export type CalendarRoomProductStatus = 'online' | 'offline'
...
export type CalendarRoomProduct = {
  id: string
  name: string
  channel: string
  breakfast: string
  refund: string
  pricePlan: string
  status: CalendarRoomProductStatus
  actions: Array<'预览' | '编辑' | '修改价格' | '上架' | '下架'>
}
...
export type CalendarRoomViewModel = {
  providerMode: CalendarRoomProviderMode
  responseState: CalendarRoomMockState
  endpoint: string
  traceId: string
  timestamp: string
...
type CalendarRoomBackendData = Omit<
  CalendarRoomViewModel,
...
export async function fetchCalendarRoomProducts(query: CalendarRoomQuery, signal?: AbortSignal): Promise<CalendarRoomViewModel> {
...
async function fetchMockCalendarRoom(
  query: CalendarRoomQuery,
  signal?: AbortSignal,
): Promise<ApiEnvelope<CalendarRoomBackendData>> {
...
async function fetchRealCalendarRoom(query: CalendarRoomQuery, signal?: AbortSignal): Promise<CalendarRoomViewModel> {
  ...
  if (productTotal === 0) {
    ...
    const fallbackEnvelope = await postRealCalendarRoom<RealRoomCategoryPayload>(ROOM_CATEGORY_FALLBACK_ENDPOINT, fallbackRequestParams, signal)
...
function createBackendData(
  ...
  rows: CalendarRoomRow[],
  channelCatalog: ChannelCatalogItem[],
  optionSourceRows = rows,
  associatedChannelNames: string[] = [DEFAULT_LOCAL_CHANNEL_NAME],
): CalendarRoomBackendData {
  return {
...
function buildRoomCategoryFallbackRequestParams(query: CalendarRoomQuery, campId: string) {
  const storeId = query.storeId?.trim()
  return {
    campId,
...
function adaptRealCalendarRoomRows(
  input: unknown,
  channelCatalog: ChannelCatalogItem[],
  associatedChannelNames: string[],
): CalendarRoomRow[] {
...
function adaptRoomCategoryFallbackRows(
  input: unknown,
  channelCatalog: ChannelCatalogItem[],
  associatedChannelNames: string[],
): CalendarRoomRow[] {
  return asArray(input).map((item, index) => {
    ...
    const roomName = readString(
      record.roomCategoryName ?? record.roomTypeName ?? record.internalName ?? record.name,
      `未命名房型${index + 1}`,
    )
    const status: CalendarRoomProductStatus = readBookingStatus(record.isAvailability ?? record.status, true) ? 'online' : 'offline'
    const product: CalendarRoomProduct = {
      id: `local-${roomId}`,
      name: roomName,
...
function filterRows(rows: CalendarRoomRow[], query: CalendarRoomQuery) {
  ...
  return rows
    ...
    .filter((row): row is CalendarRoomRow => row !== null)
}
...
async function fetchCalendarRoomChannelCatalog(provider: CustomChannelProviderName) {
...
function buildProductChannelBadges(
  products: CalendarRoomProduct[],
  channelCatalog: ChannelCatalogItem[],
  associatedChannelNames: string[] = [DEFAULT_LOCAL_CHANNEL_NAME],
) {
...
function buildChannelBadge(channelName: string, channelCatalog: ChannelCatalogItem[]): CalendarRoomChannelBadge {
  ...
  return createGeneratedChannelBadge(
    catalogItem ?? {
...
function findChannelCatalogItem(channelName: string, channelCatalog: ChannelCatalogItem[]) {
...
function createGeneratedChannelBadge(channel: ChannelCatalogItem): CalendarRoomChannelBadge {
...
function createChannelIcon(channel: ChannelCatalogItem) {
...
function toChannelBadgeKey(channel: string): keyof typeof CHANNEL_BADGE_LIBRARY | '' {
  ...
  if (channel.includes('宿银') || channel.includes('路客') || channel.includes('聚合') || channel.includes('本地')) return 'locals'
  return ''
}
...
function readProductChannel(...records: Array<Record<string, unknown>>) {
  for (const record of records) {
    ...
    if (channel) return normalizeDisplayChannelName(channel)
  }
  return DEFAULT_LOCAL_CHANNEL_NAME
}

function normalizeDisplayChannelName(channelName: string) {
...
function readRoomCategoryPricePlan(record: Record<string, unknown>, fallback: string) {
...
function product(
  ...
  channel: string,
  pricePlan: string,
  status: CalendarRoomProductStatus = 'online',
  actions?: CalendarRoomProduct['actions'],
): CalendarRoomProduct {
  return {
    id,
    name,
