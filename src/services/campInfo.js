...
const DEFAULT_QUERY = { keyword: '', page: 1, pageSize: 20 };
...
export async function fetchCampInfoOverview(query = {}, signal) {
...
export async function fetchCampInfoDetail(storeId, signal) {
    const provider = resolveProvider();
    if (provider === 'api') {
...
export async function createCampInfoImportTask(optionId) {
    const option = importOptions.find((item) => item.id === optionId);
    if (!option)
        throw new CampInfoRequestError('未找到导入选项');
...
export async function saveCampInfoSort(activeTab, itemIds) {
    if (resolveProvider() === 'api') {
...
function normalizeQuery(query) {
    return {
        ...
        pageSize: Number.isFinite(query.pageSize) && Number(query.pageSize) > 0 ? Number(query.pageSize) : DEFAULT_QUERY.pageSize,
    };
}
async function fetchMockOverview(query) {
    await waitForMockLatency();
    ...
    const emptyMessage = mode === 'empty'
        ...
        : stores.length === 0
            ? '暂无符合条件的门店'
            : '暂无已创建的门店';
    const envelope = createMockEnvelope('mock-shezhi--xinxi-weihu--mendian-xinxi-list-001', {
        list: stores,
        pagination: {
            page: query.page,
...
function createMockEnvelope(traceId, data) {
    return {
        code: 0,
        message: 'success',
        data,
...
function readCampId(value) {
    const record = readRecord(value);
    const camps = Array.isArray(record?.camps) ? record.camps : Array.isArray(value?.camps) ? value.camps : [];
