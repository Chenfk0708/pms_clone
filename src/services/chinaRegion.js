...
function parseRegionDataSource(source, isRecord, label) {
...
function getProvinceCode(record) {
    return `${record.province}0000`;
}
function getCityCode(record) {
...
function buildRegionTree() {
    ...
    const districtRecordsByCityCode = groupRecordsBy(districtRecords, (record) => SINGLE_CITY_PROVINCE_CODES.has(getProvinceCode(record)) ? getProvinceCode(record) : getCityCode(record));
...
export function getRegionPanelState(selection) {
...
export function resolveRegionSelection(selection) {
...
export function formatRegionLabel(selection) {
