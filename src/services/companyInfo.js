...
let mockProfileState = cloneProfile(defaultProfile);
...
export const defaultCompanyInfoQuery = {
    campId: DEFAULT_CAMP_ID,
    includeImages: true,
};
export async function fetchCompanyInfo(query = defaultCompanyInfoQuery, signal) {
...
export async function saveCompanyInfo(profile) {
    ...
    if (provider === 'mock') {
        ...
        return buildViewModel(provider, defaultCompanyInfoQuery, mockProfileState, `mock-${TASK_ID}-save-001`, RESPONSE_TIMESTAMP);
    }
...
export function createEmptyCompanyInfoDraft() {
    return {
        name: '',
        type: '民宿',
        phone: '',
...
export function createUploadedCompanyImage(existingImages) {
...
async function waitForMockCompanyInfo(query) {
    ...
    return buildViewModel('mock', query, data, `mock-${TASK_ID}-${mode === 'empty' ? 'empty' : 'detail'}-001`, RESPONSE_TIMESTAMP);
}
function buildViewModel(provider, requestBody, input, traceId, timestamp) {
...
function resolveMockMode() {
    ...
    if (fromUrl)
        return fromUrl;
    const configured = readRuntimeConfig('pms.companyInfo.mockMode') ||
        import.meta.env.VITE_COMPANY_INFO_MOCK_MODE;
