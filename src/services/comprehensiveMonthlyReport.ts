...
export type ComprehensiveMonthlyReportQuery = {
  campId: string
  startDate: string
  endDate: string
  page: number
  pageSize: number
  provider?: ComprehensiveMonthlyReportProvider
  mockState?: ComprehensiveMonthlyReportMockState
}

export type ComprehensiveMonthlyReportSelection = {
  startDate: string
  endDate: string
}

export type ComprehensiveMonthlyReportListRow = {
  id: string
  monthLabel: string
  rangeLabel: string
  revenueText: string
...
export type ComprehensiveMonthlyReportListData = {
  provider: ComprehensiveMonthlyReportProvider
  state: ComprehensiveMonthlyReportMockState
  endpoint: string
  requestBody: Record<string, unknown>
  traceId: string
  timestamp: string
  rows: ComprehensiveMonthlyReportListRow[]
  pagination: {
    page: number
    pageSize: number
    total: number
...
type ComprehensiveMonthlyReportEnvelope<T> = {
  code: number
  message: string
  data: T
  traceId: string
...
type ComprehensiveMonthlyReportEnvelopeData = {
  list: ComprehensiveMonthlyReportTargetItem[]
  pagination: {
    page: number
    pageSize: number
    total: number
...
type ComprehensiveMonthlyReportTargetItem = {
  date: string
  startDate: number
  endDate: number
  includeCommissionRoomPrice: number
  orderOtherExpense: number
...
export class ComprehensiveMonthlyReportServiceError extends Error {
  response: ComprehensiveMonthlyReportEnvelope<ComprehensiveMonthlyReportEnvelopeData | null>

  constructor(response: ComprehensiveMonthlyReportEnvelope<ComprehensiveMonthlyReportEnvelopeData | null>) {
...
const mockItems: ComprehensiveMonthlyReportTargetItem[] = [
...
export function createDefaultComprehensiveMonthlyReportQuery(
...
export function resolveComprehensiveMonthlyRuntimeConfig(search: string): {
  provider: ComprehensiveMonthlyReportProvider
  mockState: ComprehensiveMonthlyReportMockState
} {
...
export function readComprehensiveMonthlySelection(search: string): ComprehensiveMonthlyReportSelection | null {
  const params = new URLSearchParams(search)
...
export async function loadComprehensiveMonthlyReportList(
  query: ComprehensiveMonthlyReportQuery,
  signal?: AbortSignal,
): Promise<ComprehensiveMonthlyReportListData> {
...
export function findComprehensiveMonthlyReportRow(
  rows: ComprehensiveMonthlyReportListRow[],
  selection: ComprehensiveMonthlyReportSelection | null,
): ComprehensiveMonthlyReportListRow | null {
  if (!selection) return rows[0] ?? null
  return rows.find((row) => row.startDate === selection.startDate && row.endDate === selection.endDate) ?? null
}

export async function runComprehensiveMonthlyReportAction(
  action: ComprehensiveMonthlyReportAction,
  provider: ComprehensiveMonthlyReportProvider,
  signal?: AbortSignal,
): Promise<{
  traceId: string
...
async function loadFromApi(
  ...
  const payload = (await response.json().catch(() => null)) as HudsonResponse<HudsonMonthlyPageData> | null
...
function adaptEnvelope(
  envelope: ComprehensiveMonthlyReportEnvelope<ComprehensiveMonthlyReportEnvelopeData | null>,
  requestBody: Record<string, unknown>,
  provider: ComprehensiveMonthlyReportProvider,
  state: ComprehensiveMonthlyReportMockState,
): ComprehensiveMonthlyReportListData {
...
function adaptRow(item: ComprehensiveMonthlyReportTargetItem): ComprehensiveMonthlyReportListRow {
...
function adaptTargetItem(value: unknown): ComprehensiveMonthlyReportTargetItem {
...
function createRequestBody(query: ComprehensiveMonthlyReportQuery) {
  return {
    campId: query.campId,
...
): ComprehensiveMonthlyReportTargetItem {
  return {
    date,
...
function readMockStateFromStorage(): ComprehensiveMonthlyReportMockState | null {
  if (typeof window === 'undefined') return null
  const configured =
    window.localStorage.getItem('pms.comprehensiveMonthlyReportMockState')?.trim() ||
    import.meta.env.VITE_COMPREHENSIVE_MONTHLY_REPORT_MOCK_STATE
  return normalizeMockState(configured)
}

function normalizeMockState(value: string | null | undefined): ComprehensiveMonthlyReportMockState | null {
...
function formatDateTimeWithBreak(timestamp: number) {
  const date = new Date(timestamp)
