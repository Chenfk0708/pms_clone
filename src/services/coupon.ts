export type CouponProviderName = 'mock' | 'api'
type CouponMockMode = 'success' | 'empty' | 'error'

export type CouponShelfStatus = 'all' | 'enabled' | 'disabled'
...
export type CouponListFilters = {
  ...
  pageNum: number
  pageSize: number
  provider?: CouponProviderName
}

export type CouponTaskFilters = {
  campId: string
  pageNum: number
  pageSize: number
  provider?: CouponProviderName
...
export type CouponPageData<T> = {
  list: T[]
  pagination: { pageNum: number; pageSize: number; total: number; pages: number }
}

export type CouponViewModel<T> = CouponPageData<T> & {
  endpoint: string
...
type TargetCouponResponse = Partial<CouponEnvelope<unknown>> & {
  success?: boolean
...
export async function fetchCouponList(filters: CouponListFilters, signal?: AbortSignal): Promise<CouponViewModel<CouponRow>> {
  const requestBody = createCouponListRequestBody(filters)
  const provider = resolveCouponProviderName(filters.provider)

  if (provider === 'mock') return fetchMockCouponList(filters, requestBody)
...
export async function fetchCouponTasks(filters: CouponTaskFilters, signal?: AbortSignal): Promise<CouponViewModel<CouponTaskRow>> {
  const requestBody = createCouponTaskRequestBody(filters)
  const provider = resolveCouponProviderName(filters.provider)

  if (provider === 'mock') return fetchMockCouponTasks(requestBody)
...
export function createCouponListRequestBody(filters: CouponListFilters): Record<string, unknown> {
  return {
...
export function createCouponTaskRequestBody(filters: CouponTaskFilters): Record<string, unknown> {
  return {
    campId: filters.campId,
    pageNum: filters.pageNum,
    pageSize: filters.pageSize,
...
function fetchMockCouponList(filters: CouponListFilters, requestBody: Record<string, unknown>) {
  ...
  if (response.code !== 0) throw new CouponRequestError()
  const adapted = adaptCouponPage(response.data, requestBody, 'mock', couponListEndpoint, response, adaptCouponRow)
  const filteredList =
    filters.shelfStatus === 'all' ? adapted.list : adapted.list.filter((item) => item.status === (filters.shelfStatus === 'enabled' ? '已上架' : '已下架'))
  return {
    ...adapted,
    list: filteredList,
...
function fetchMockCouponTasks(requestBody: Record<string, unknown>) {
  const mode = resolveCouponMockMode()
  const response =
    mode === 'error'
      ? mockCouponErrorEnvelope()
  ...
  if (response.code !== 0) throw new CouponRequestError('派发任务数据加载失败')
  return adaptCouponPage(response.data, requestBody, 'mock', couponTaskEndpoint, response, adaptCouponTaskRow)
}

function adaptCouponPage<T>(
  data: unknown,
  requestBody: Record<string, unknown>,
  provider: CouponProviderName,
  endpoint: string,
  envelope: TargetCouponResponse | null | undefined,
  adaptRow: (value: unknown) => T | null,
): CouponViewModel<T> {
  const record = readRecord(data)
  const list = readArray(record?.list).map(adaptRow).filter((item): item is T => Boolean(item))
...
function adaptCouponRow(value: unknown): CouponRow | null {
...
function adaptCouponTaskRow(value: unknown): CouponTaskRow | null {
  const record = readRecord(value)
  if (!record) return null
  const id = readString(record.id)
  const couponName = readString(record.couponName)
  if (!id || !couponName) return null
  return {
    id,
...
function formatDiscount(record: Record<string, unknown>) {
...
function resolveCouponProviderName(explicitProvider?: CouponProviderName): CouponProviderName {
  const configured =
    explicitProvider ||
...
function resolveCouponMockMode(): CouponMockMode {
  const configured =
    readRuntimeConfig('pms.couponMockMode') ||
    (import.meta.env.VITE_PMS_COUPON_MOCK_MODE as string | undefined)
  if (configured === 'empty' || configured === 'error') return configured
  return 'success'
}

function mockCouponListSuccessEnvelope(requestBody: Record<string, unknown>): CouponEnvelope<Record<string, unknown>> {
  return {
    code: 0,
    message: 'success',
    data: {
...
function mockCouponTaskSuccessEnvelope(requestBody: Record<string, unknown>): CouponEnvelope<Record<string, unknown>> {
  return {
    code: 0,
    message: 'success',
    data: {
...
function mockCouponListEmptyEnvelope(requestBody: Record<string, unknown>): CouponEnvelope<Record<string, unknown>> {
  return {
    ...mockCouponListSuccessEnvelope(requestBody),
...
function mockCouponTaskEmptyEnvelope(requestBody: Record<string, unknown>): CouponEnvelope<Record<string, unknown>> {
  return {
    ...mockCouponTaskSuccessEnvelope(requestBody),
    data: { total: 0, size: 20, current: 1, pageNum: 1, pages: 0, hasNextPage: false, list: [] },
...
function mockCouponErrorEnvelope(): CouponEnvelope<null> {
  return {
    code: 50001,
...
async function readJson(response: Response): Promise<TargetCouponResponse | null> {
  try {
    return (await response.json()) as TargetCouponResponse
  } catch {
    return null
  }
}

function isFailedResponse(payload: TargetCouponResponse | null) {
  if (!payload) return false
  if (payload.code !== undefined) return payload.code !== 0
  return payload.success === false
}

function extractErrorMessage(payload: TargetCouponResponse | null) {
