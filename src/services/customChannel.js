...
export async function fetchCustomChannelDashboard(query) {
    ...
    if (query.provider === 'api') {
        const data = await apiPost(CUSTOM_CHANNEL_LIST_PATH, {});
...
export async function saveSystemChannels(enabledMap, query) {
    ...
    if (query.provider === 'api') {
        const systemChannels = Object.entries(enabledMap).map(([id, enabled]) => ({ id, enabled }));
        const data = await apiPost(CUSTOM_CHANNEL_UPDATE_PATH, { systemChannels });
...
export async function createCustomChannel(input, query) {
    ...
    if (query.provider === 'api') {
        const data = await apiPost(CUSTOM_CHANNEL_CREATE_PATH, input);
...
export async function updateCustomChannel(channelId, input, query) {
    writeDiagnostics({ action: 'update', provider: query.provider, updatePath: CUSTOM_CHANNEL_UPDATE_PATH, channelId, payload: input });
    validateDialogInput(input);
    if (query.provider === 'api') {
        const data = await apiPost(CUSTOM_CHANNEL_UPDATE_PATH, { channelId, ...input });
        return adaptApiDashboard(data, query, 'update');
    }
    await wait(MOCK_LATENCY_MS);
    mockCustomChannels = mockCustomChannels.map((channel) => channel.id === channelId ? { ...channel, name: input.name.trim(), color: input.color, colorName: input.colorName, updatedAt: '2026-05-20 00:30:00' } : channel);
    return adaptMockDashboard({ systemChannels: mockSystemChannels, customChannels: mockCustomChannels }, query, 'update-001');
}
export async function toggleCustomChannelStatus(channelId, nextEnabled, query) {
    writeDiagnostics({ action: 'toggle', provider: query.provider, updatePath: CUSTOM_CHANNEL_UPDATE_PATH, channelId, nextEnabled });
    if (query.provider === 'api') {
        const data = await apiPost(CUSTOM_CHANNEL_UPDATE_PATH, { channelId, enabled: nextEnabled ? 1 : 0 });
        return adaptApiDashboard(data, query, 'toggle');
    }
    await wait(MOCK_LATENCY_MS);
    mockCustomChannels = mockCustomChannels.map((channel) => channel.id === channelId ? { ...channel, enabled: nextEnabled, updatedAt: '2026-05-20 00:30:00' } : channel);
    return adaptMockDashboard({ systemChannels: mockSystemChannels, customChannels: mockCustomChannels }, query, 'toggle-001');
}
export async function deleteCustomChannel(channelId, query) {
    writeDiagnostics({ action: 'delete', provider: query.provider, deletePath: CUSTOM_CHANNEL_DELETE_PATH, channelId });
    if (query.provider === 'api') {
        const data = await apiPost(CUSTOM_CHANNEL_DELETE_PATH, { channelId });
...
function adaptApiDashboard(payload, query, action) {
    return {
...
function adaptMockDashboard(payload, query, traceSuffix) {
    return {
        provider: query.provider,
        state: query.mockState,
...
function getCustomChannelProviderName() {
    if (typeof window === 'undefined')
        return 'api';
    const fromEnv = import.meta.env.VITE_CUSTOM_CHANNEL_PROVIDER;
    if (fromEnv === 'api' || fromEnv === 'mock')
        return fromEnv;
