...
export async function saveSystemChannels(enabledMap, query) {
    ...
    return adaptDashboard(createSuccessEnvelope('save-system-001', {
...
export async function createCustomChannel(input, query) {
    ...
    return adaptDashboard(createSuccessEnvelope('create-001', {
        systemChannels: mockSystemChannels,
        customChannels: mockCustomChannels,
    }), { ...query, mockState: 'success' });
}
export async function updateCustomChannel(channelId, input, query) {
    ...
    validateDialogInput(input);
    await wait(MOCK_LATENCY_MS);
    mockCustomChannels = mockCustomChannels.map((channel) => channel.id === channelId
        ? {
            ...channel,
            name: input.name.trim(),
            color: input.color,
        ...
        : channel);
    return adaptDashboard(createSuccessEnvelope('update-001', {
        systemChannels: mockSystemChannels,
        customChannels: mockCustomChannels,
    }), { ...query, mockState: 'success' });
}
export async function toggleCustomChannelStatus(channelId, nextEnabled, query) {
    ...
    mockCustomChannels = mockCustomChannels.map((channel) => channel.id === channelId
        ...
        : channel);
    return adaptDashboard(createSuccessEnvelope('toggle-001', {
        systemChannels: mockSystemChannels,
        customChannels: mockCustomChannels,
    }), { ...query, mockState: 'success' });
}
export async function deleteCustomChannel(channelId, query) {
    ...
    return adaptDashboard(createSuccessEnvelope('delete-001', {
        systemChannels: mockSystemChannels,
        customChannels: mockCustomChannels,
    }), { ...query, mockState: 'success' });
}
function adaptDashboard(envelope, query) {
    return {
        provider: query.provider,
...
function createSuccessEnvelope(traceSuffix, data) {
    return {
        code: 0,
        message: 'success',
        data,
...
function writeDiagnostics(value) {
    if (typeof window === 'undefined')
        return;
    window.localStorage.setItem(CUSTOM_CHANNEL_LAST_REQUEST_KEY, JSON.stringify({
        ...value,
        timestamp: MOCK_TIMESTAMP,
    }));
}
