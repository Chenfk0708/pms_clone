...
export function resolveCustomerAddBatchRuntimeConfig(location) {
...
export function getDefaultCustomerAddBatchQuery(overrides = {}) {
    return {
...
export async function fetchCustomerAddBatchDashboard(query, signal) {
...
export function createCustomerAddBatchExportTask(query) {
...
export function createCustomerAddBatchSmsTask(candidate, query) {
    writeDiagnostics({
...
export function createCustomerAddBatchMarkTask(candidate, query) {
    writeDiagnostics({
...
async function fetchMockDashboard(query, signal) {
...
async function fetchApiDashboard(query, signal) {
...
function createBackendData(query, candidates) {
...
function filterCandidates(rows, query) {
    return rows.filter((row) => {
        if (query.channel && row.sourceChannel !== query.channel)
            return false;
        if (query.friendStatus && row.friendStatus !== query.friendStatus)
            return false;
        return true;
    });
}
function metric(key, label, value, unit, description) {
    return {
        key,
        label,
        value: String(value),
