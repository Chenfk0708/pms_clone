...
export const defaultGlobalSettingFilters = {
...
export async function fetchGlobalSettingOverview(filters, signal) {
...
export async function fetchGlobalSettingStoreConfig(storeId) {
    if (resolveProvider() === 'api') {
        const payload = await postJson('/radarConfig/shop/config/get', { storeId });
        const data = readRecord(payload?.data);
        if (!data)
            throw new GlobalSettingRequestError('配置详情响应缺失');
        return adaptStoreConfig(data, storeId);
    }
    const config = storeConfigs[storeId];
    if (!config)
        throw new GlobalSettingRequestError('未找到门店配置');
    return cloneConfig(config);
}
export async function saveGlobalSettingStoreSelection(current, selectedPoiIds) {
    if (resolveProvider() === 'api') {
    ...
    return buildOverviewViewModel(buildStoresFromPoiIds(selectedPoiIds), normalizeCandidates(selectedPoiIds), current.filters, current.requestBody, current.provider, 'mock-ai-quanyu-leida--shuju-yu-peizhi--peizhi-zhongxin-selection-001');
}
export async function saveGlobalSettingStoreConfig(current, detail) {
    const nextDetail = {
        ...detail,
    ...
    storeSeeds[nextDetail.poiId] = {
        ...
        meituanAuthStatus: nextDetail.meituan.enabled
            ? nextDetail.meituan.username && nextDetail.meituan.password
                ...
                : 'failed'
            : 'unauthorized',
        connectorStatus: nextDetail.ctrip.enabled || nextDetail.meituan.enabled
            ? nextDetail.ctrip.authStatus === 'failed' || nextDetail.meituan.authStatus === 'failed'
                ? 'warning'
                : 'online'
            : 'offline',
        monitorStatus: nextDetail.ctrip.authStatus === 'authorized' || nextDetail.meituan.authStatus === 'authorized' ? 'checking' : 'delay',
    ...
    const nextViewModel = buildOverviewViewModel(current.stores.map((item) => storeSeeds[item.poiId]), current.candidates, current.filters, current.requestBody, current.provider, 'mock-ai-quanyu-leida--shuju-yu-peizhi--peizhi-zhongxin-config-save-001');
    return { viewModel: nextViewModel, detail: cloneConfig(nextDetail) };
}
export async function removeGlobalSettingStore(current, storeId) {
    if (resolveProvider() === 'api') {
    ...
    return buildOverviewViewModel(nextStores, normalizeCandidates(selectedPoiIds), current.filters, current.requestBody, current.provider, 'mock-ai-quanyu-leida--shuju-yu-peizhi--peizhi-zhongxin-remove-001');
}
...
export async function createGlobalSettingExportTask(filters) {
    if (resolveProvider() === 'api') {
...
export function createGlobalSettingRequestBody(filters) {
    return {
...
function fetchMockOverview(filters, requestBody) {
    return waitForMockOverview(filters, requestBody);
}
async function waitForMockOverview(filters, requestBody) {
    ...
    return buildOverviewViewModel(stores, normalizeCandidates(selectedPoiIds), filters, requestBody, 'mock', 'mock-ai-quanyu-leida--shuju-yu-peizhi--peizhi-zhongxin-overview-001');
}
function adaptOverview(data, filters, requestBody, provider, envelope) {
    const record = readRecord(data);
    const stores = readArray(record?.stores).map(adaptStoreRow).filter((item) => Boolean(item));
    const candidates = readArray(record?.candidates)
        .map(adaptCandidate)
        .filter((item) => Boolean(item));
    return buildOverviewViewModel(stores.length > 0 ? stores.map((item) => storeSeeds[item.poiId] ?? item) : [], candidates.length > 0 ? candidates : normalizeCandidates(stores.map((item) => item.poiId)), filters, requestBody, provider, envelope?.traceId ?? `${provider}-ai-quanyu-leida--shuju-yu-peizhi--peizhi-zhongxin-overview`, envelope?.timestamp ?? timestamp);
}
function buildOverviewViewModel(sourceStores, candidates, filters, requestBody, provider, traceId, responseTimestamp = timestamp) {
...
function matchesFilters(row, filters) {
    ...
    if (filters.connectorStatus !== 'all' && row.connectorStatus !== filters.connectorStatus)
        return false;
    if (filters.authorizationStatus !== 'all') {
        if (filters.authorizationStatus === 'authorized' && !isAuthorizedStore(row))
            return false;
        if (filters.authorizationStatus === 'warning' && !hasFailedAuthorization(row))
            return false;
        if (filters.authorizationStatus === 'unauthorized' && isAuthorizedStore(row))
            return false;
    }
    if (keyword && !`${row.name}${row.city}${row.enabledChannels.join('')}`.includes(keyword))
        return false;
    return true;
}
function isAuthorizedStore(row) {
    return row.ctripAuthStatus === 'authorized' && row.meituanAuthStatus === 'authorized';
}
function hasFailedAuthorization(row) {
    return row.ctripAuthStatus === 'failed' || row.meituanAuthStatus === 'failed';
}
function buildStoresFromPoiIds(selectedPoiIds) {
    return selectedPoiIds.map((poiId) => storeSeeds[poiId]).filter((item) => Boolean(item));
}
function normalizeCandidates(selectedPoiIds) {
...
function createConfigRequestBody(detail) {
    return {
...
function adaptStoreRow(value) {
    const record = readRecord(value);
    const id = readString(record?.id);
    const poiId = readString(record?.poiId);
    const name = readString(record?.name);
    if (!record || !id || !poiId || !name)
        return null;
    return {
        id,
        poiId,
...
function adaptCandidate(value) {
    const record = readRecord(value);
    const poiId = readString(record?.poiId);
    const name = readString(record?.name);
    if (!record || !poiId || !name)
        return null;
    return {
        poiId,
        campId: readString(record.campId) ?? 'camp-up-valley',
...
function adaptStoreConfig(value, storeId) {
    return {
        storeId,
...
function adaptChannelConfig(value) {
    return {
...
function resolveMockMode() {
    const configured = readRuntimeConfig('pms.globalSettingMockMode') ||
        import.meta.env.VITE_PMS_GLOBAL_SETTING_MOCK_MODE;
...
function readConnectorStatus(value) {
    return value === 'warning' || value === 'offline' ? value : 'online';
}
function readMonitorStatus(value) {
