...
export type HotelPackageOrderOption = {
  value: string
  label: string
}

export type HotelPackageOrderRow = {
  id: string
  productName: string
  quantity: string
  unitPrice: string
  schedulePriceDiff: string
  paidAmount: string
  contact: string
  orderState: string
...
export type HotelPackageOrderData = {
  provider: HotelPackageOrderProvider
  traceId: string
  endpoint: string
  requestBody: Record<string, unknown>
  rows: HotelPackageOrderRow[]
  pagination: {
    page: number
    pageSize: number
    total: number
  ...
  options: {
    sources: HotelPackageOrderOption[]
    afterSales: HotelPackageOrderOption[]
    orderStates: HotelPackageOrderOption[]
  }
  requestedAt: string
}

export type HotelPackageOrderLoadResult =
  | { ok: true; data: HotelPackageOrderData }
  | { ok: false; provider: HotelPackageOrderProvider; endpoint: string; requestBody: Record<string, unknown>; message: string }

type ResponsePackage<T> = {
  code: number
  message: string
  data: T
  traceId: string
...
type HotelPackageOrderListPayload = {
  list: Array<{
    orderId: string
    roomCategoryName: string
    count: number
    unitPrice: number
    schedulePriceDiff: number
    paidAmount: number
    contactPhone: string
    orderState: string
    refundDisplayState: string
  ...
  options: HotelPackageOrderData['options']
}
...
const orderStateOptions: HotelPackageOrderOption[] = [
  { value: 'all', label: '全部' },
...
const sourceOptions: HotelPackageOrderOption[] = [
...
const afterSaleOptions: HotelPackageOrderOption[] = [
  { value: 'none', label: '无售后' },
  { value: 'refunding', label: '退款中' },
  { value: 'refunded', label: '退款成功' },
]

const baseRows: HotelPackageOrderListPayload['list'] = [
  {
...
export function readInitialHotelPackageOrderFilters(): HotelPackageOrderFilters {
...
export async function loadHotelPackageOrderData(
  filters: HotelPackageOrderFilters,
  options?: { signal?: AbortSignal; mockState?: HotelPackageOrderMockState },
): Promise<HotelPackageOrderLoadResult> {
  const provider = getHotelPackageOrderProvider()
  const requestBody = createHotelPackageOrderRequestBody(filters)

  if (provider === 'api') return loadHotelPackageOrderFromApi(requestBody, options?.signal)
  return loadHotelPackageOrderFromMock(filters, requestBody, options?.mockState ?? getHotelPackageOrderMockState())
}

async function loadHotelPackageOrderFromMock(
  filters: HotelPackageOrderFilters,
  requestBody: Record<string, unknown>,
  state: HotelPackageOrderMockState,
): Promise<HotelPackageOrderLoadResult> {
  await delay(180)
  if (state === 'error') {
    return {
...
async function loadHotelPackageOrderFromApi(
  requestBody: Record<string, unknown>,
  signal?: AbortSignal,
): Promise<HotelPackageOrderLoadResult> {
  try {
    ...
    if (!response.ok) throw new Error(`酒店套餐订单接口返回 HTTP ${response.status}`)
...
function createMockResponse(
  filters: HotelPackageOrderFilters,
  state: HotelPackageOrderMockState,
): ResponsePackage<HotelPackageOrderListPayload> {
  const filteredRows =
    state === 'empty'
      ? []
      : baseRows.filter((row) => {
          const sourceMatch =
            !filters.source || sourceOptions.find((option) => option.value === filters.source)?.label === row.orderChannelName
          const afterSaleMatch =
            !filters.afterSale || afterSaleOptions.find((option) => option.value === filters.afterSale)?.label === row.refundDisplayState
...
function adaptResponsePackage(
  payload: ResponsePackage<HotelPackageOrderListPayload>,
  requestBody: Record<string, unknown>,
  provider: HotelPackageOrderProvider,
): HotelPackageOrderData {
  return {
    provider,
...
function adaptHudsonPayload(payload: unknown, requestBody: Record<string, unknown>): HotelPackageOrderData {
...
function adaptMockRow(row: HotelPackageOrderListPayload['list'][number]): HotelPackageOrderRow {
  return {
    id: row.orderId,
...
function adaptHudsonRow(value: unknown, index: number): HotelPackageOrderRow {
