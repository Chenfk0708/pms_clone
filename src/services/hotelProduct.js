...
export async function loadHotelProductData(query = {}, signal) {
...
export function getHotelProductProviderName() {
...
function filterProducts(products, query) {
    ...
    return products.filter((item) => {
        ...
        if (keyword && !item.title.includes(keyword))
            return false;
        if (roomType && item.roomCategoryName !== roomType)
            return false;
        if (channel && item.channelName !== channel)
            return false;
        return true;
    });
}
...
async function loadRealHotelProductData(query, signal) {
...
function createRealListRequest(campId, query) {
    return {
        campId,
...
function adaptRealProducts(data) {
    const record = asRecord(data);
