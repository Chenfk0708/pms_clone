...
export type HouseDaysRoomBooking = {
  ...
  channel: string
  price: string
  tone: 'pending' | 'live' | 'checkout' | 'duplicate'
...
function buildStoreOptions(rooms: HouseDaysRoomCard[]) {
  const stores = new Map<string, string>()
  for (const room of rooms) {
    if (room.storeId && room.storeId !== 'all') stores.set(room.storeId, room.storeName || `门店 ${stores.size + 1}`)
  }
...
function resolveDayBookingTone(tone: DayOrderBooking['cell']['tone']): HouseDaysRoomBooking['tone'] {
  if (tone === 'booking-duplicate') return 'duplicate'
  if (tone === 'booking-live') return 'live'
  if (tone === 'booking-checkout') return 'checkout'
  return 'pending'
}
...
function buildStatusGroups(rooms: HouseDaysRoomCard[], basic?: Record<string, unknown>): HouseDaysStatusGroup[] {
  ...
  const countByTone = (tone: HouseDaysRoomBooking['tone']) =>
    rooms.filter((room) => getRoomBookings(room).some((booking) => booking.tone === tone)).length
