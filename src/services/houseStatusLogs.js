...
export async function fetchHouseStatusLogs(query) {
...
export function resolveHouseStatusLogsProvider() {
    const configuredProvider = readConfiguredProvider();
    if (configuredProvider === 'real' || configuredProvider === 'mock')
        return configuredProvider;
    return 'mock';
}
async function fetchRealHouseStatusLogs(query) {
    if (!query.campId) {
        return {
            code: 400,
...
async function fetchMockHouseStatusLogs(query) {
    await delay(120);
...
function adaptHouseStatusLogResponse(response, provider) {
    if (response.code !== 0) {
...
function readConfiguredProvider() {
    ...
    return import.meta.env.VITE_HOUSE_STATUS_LOGS_PROVIDER;
}
function emptyResponseData(query) {
    return {
        list: [],
        pagination: {
            page: query.pageNum,
...
function filterMockLogs(query) {
    ...
    return mockHouseStatusLogs.filter((log) => {
        if (query.adjustType && adjustmentTypeByText[log.adjustContent || ''] !== query.adjustType)
            return false;
        if (selectedChannelName && !log.channelRoomStatusOperationLogViews?.some((item) => item.channelName === selectedChannelName))
            return false;
        if (query.userName && !log.userName?.includes(query.userName))
            return false;
        if (!keyword)
            return true;
...
function isHouseStatusLogRecord(value) {
