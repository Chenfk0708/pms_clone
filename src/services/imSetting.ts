...
export type ImSettingProvider = 'mock' | 'api'
export type ImSettingMockState = 'success' | 'empty' | 'error'
export type ImSettingTabKey = 'phrases' | 'autoReply' | 'page' | 'tags' | 'shortcuts' | 'version'

export type ImPhraseGroup = {
  id: string
  name: string
  count: number
}

export type ImPhraseItem = {
  id: string
  title: string
  content: string
  groupId: string
...
export type ImVersionButton = {
  text: string
  type: string
  action: string
}

export type ImSettingViewModel = {
  provider: ImSettingProvider
  state: ImSettingMockState
  phraseGroups: ImPhraseGroup[]
  phrases: ImPhraseItem[]
  shortcuts: ImShortcutItem[]
  ...
  version: {
    ...
    modalTitle: string
    modalInfo: string
    buttons: ImVersionButton[]
  }
}

export type ImSettingQuery = {
  provider?: ImSettingProvider
  mockState?: ImSettingMockState
  campId: string
  userId: string
  keyword: string
  groupId: string | null
...
export type ImSettingDiagnostics = {
  provider: ImSettingProvider
  state: ImSettingMockState
  currentTab: ImSettingTabKey
  requestSummary: {
    campId?: string
    userId?: string
    keyword?: string
...
type PhraseGroupResponse = {
  imWordsGroupGetViews?: Array<{
    imWordsGroupId?: string
    name?: string
    children?: Array<{
      imWordsGroupId?: string
      name?: string
...
type MenuOptionResponse = {
  optionJsonViews?: Array<{
    optionJson?: {
      versionModals?: {
        title?: string
        info?: string
...
export async function fetchImSettingView(query: ImSettingQuery, signal?: AbortSignal): Promise<ImSettingViewModel> {
  ...
  if (provider === 'mock') {
    ...
    const normalizedState: ImSettingMockState = state === 'empty' || filteredPhrases.length === 0 ? 'empty' : 'success'
    const view: ImSettingViewModel = {
      provider,
  ...
  const [groupPayload, phrasePayload, shortcutPayload, commonsPayload, accountPayload, editionPayload, menuPayload] =
    await Promise.all([
      postJson<SuccessEnvelope<PhraseGroupResponse>>(IM_PHRASE_GROUP_ENDPOINT, phraseGroupRequest, signal),
      postJson<SuccessEnvelope<PhraseListResponse>>(IM_PHRASE_LIST_ENDPOINT, phraseListRequest, signal),
      postJson<SuccessEnvelope<ShortcutResponse>>(IM_SHORTCUT_GET_ENDPOINT, shortcutRequest, signal),
      postJson<SuccessEnvelope<CommonsResponse>>(IM_COMMONS_ENDPOINT, commonsRequest, signal),
      postJson<SuccessEnvelope<ImAccountResponse>>(IM_ACCOUNT_ENDPOINT, imAccountRequest, signal),
      postJson<SuccessEnvelope<EditionResponse>>(IM_EDITION_ENDPOINT, editionRequest, signal),
      postJson<SuccessEnvelope<MenuOptionResponse>>(IM_MENU_OPTION_ENDPOINT, menuOptionRequest, signal),
    ])
...
export async function saveImShortcutSettings(
  query: Pick<ImSettingQuery, 'userId'>,
  shortcuts: ImShortcutItem[],
  signal?: AbortSignal,
) {
...
export function resolveImSettingProvider(): ImSettingProvider {
  return readRuntimeConfig('pms.imSettingProvider') === 'api' ? 'api' : 'mock'
}

export function resolveImSettingMockState(): ImSettingMockState {
  const state = readRuntimeConfig('pms.imSettingMockState')
  if (state === 'empty' || state === 'error') return state
  return 'success'
}

export function updateImSettingDiagnostics(patch: Partial<ImSettingDiagnostics>) {
  if (typeof window === 'undefined') return
  const current = readImSettingDiagnostics()
  const next: ImSettingDiagnostics = {
...
export function readImSettingDiagnostics(): ImSettingDiagnostics | null {
  if (typeof window === 'undefined') return null
  const raw = window.localStorage.getItem(DIAGNOSTICS_KEY)
  if (!raw) return null
  try {
    return JSON.parse(raw) as ImSettingDiagnostics
  } catch {
    return null
  }
}

function buildPhraseListRequest(query: ImSettingQuery) {
  return {
    campId: query.campId,
    pageNum: 1,
    pageSize: 10,
...
function buildRequestSummary(query: ImSettingQuery) {
  return {
    campId: query.campId,
    userId: query.userId,
    keyword: query.keyword.trim(),
...
function filterMockPhrases(query: ImSettingQuery, state: ImSettingMockState) {
...
function adaptPhraseGroups(data: PhraseGroupResponse) {
...
function adaptPhrases(data: PhraseListResponse) {
...
function adaptShortcuts(data: ShortcutResponse) {
...
function unwrapBusiness<T>(payload: SuccessEnvelope<T>) {
...
const mockShortcuts: ImShortcutItem[] = [
