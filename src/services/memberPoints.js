...
export function defaultMemberPointsQuery() {
    return {
        storeId: 'store-6ts5',
...
export function resolveMemberPointsProvider() {
    const localValue = typeof window !== 'undefined' ? window.localStorage.getItem('pms.memberPointsProvider') : null;
...
export async function fetchMemberPointsDashboard(request, signal) {
    const provider = resolveMemberPointsProvider();
    const normalizedRequest = normalizeQuery(request);
    if (provider === 'api') {
        throw new MemberPointsServiceError('会员积分数据加载失败，请稍后重试', envelope(503, 'service unavailable', null, 'api-member-points-unavailable'), normalizedRequest);
    }
    await waitForMockLatency(signal);
    const normalizedState = normalizedRequest.state ?? 'success';
    if (normalizedState === 'error') {
        throw new MemberPointsServiceError('会员积分数据加载失败，请稍后重试', envelope(503, 'member points query failed', null, 'mock-scrm--huiyuan-zhongxin--huiyuan-jifen-error-001'), normalizedRequest);
    }
...
export async function createMemberPointsExportTask(request, signal) {
    await waitForMockLatency(signal);
    const normalizedRequest = normalizeQuery(request);
...
function makeOverviewEnvelope(state) {
...
function makeRecordsEnvelope(request) {
...
function adaptDashboard(provider, request, overview, records) {
    assertOk(overview);
    assertOk(records);
    return {
        provider,
