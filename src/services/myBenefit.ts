...
export type MyBenefitProvider = 'mock' | 'api'
export type MyBenefitMockState = 'success' | 'empty' | 'error'
export type MyBenefitTab = 'resources' | 'services' | 'records'

export type MyBenefitQuery = {
  provider?: MyBenefitProvider
  mockState?: MyBenefitMockState
  campId: string
  activeTab: MyBenefitTab
}

export type MyBenefitOverviewCard = {
  id: string
  label: string
  value: string
  detail: string
...
export type MyBenefitServiceEntry = {
  id: string
  label: string
  description: string
  path: string
...
export type MyBenefitServiceGroup = {
  id: string
  title: string
  items: MyBenefitServiceEntry[]
}

export type MyBenefitRecord = {
  id: string
  title: string
  typeLabel: string
  sourceLabel: string
...
export type MyBenefitPlan = {
  id: string
  name: string
  price: string
  oldPrice?: string
  tag?: string
  tone: 'standard' | 'enjoy' | 'advanced' | 'pro' | 'flagship' | 'custom'
  active?: boolean
}

export type MyBenefitViewModel = {
  provider: MyBenefitProvider
  state: MyBenefitMockState
  currentVersionName: string
  versionBadge: string
  expiresAtText: string
  refreshedAt: string
  overviewCards: MyBenefitOverviewCard[]
  resources: MyBenefitResource[]
  serviceGroups: MyBenefitServiceGroup[]
  records: MyBenefitRecord[]
  plans: MyBenefitPlan[]
}

export type MyBenefitDiagnostics = {
  endpoint: string
  provider: MyBenefitProvider
  state: MyBenefitMockState
  traceId: string
...
export type MyBenefitResult = {
  view: MyBenefitViewModel
  diagnostics: MyBenefitDiagnostics
}
...
type TargetResourceResponse = {
  ...
  data: {
    ...
    editionName: string
    editionType: number
    resourceGetViews: TargetResourceItem[]
    valueAddServices: null
  }
}

type TargetResourceItem = {
  resourceName: string
  quotaNum: number | null
  isUnlimitedQuotaNum: number | null
  usedQuotaView: {
    usedQuotaNum: number
    campUsedQuotaViews: Array<{
      campId: string
      campName: string
      usedQuotaNum: number
    }>
  ...
  resourceFrom: {
    ...
    isUnlimitedEditionNum: number | null
    expandQuotaNum: number | null
    expandQuotaDetailView: Array<{
      payType: number | null
...
export function resolveMyBenefitRuntimeConfig(
  location: Pick<Location, 'search'>,
): Pick<MyBenefitQuery, 'provider' | 'mockState' | 'activeTab'> {
...
export function createDefaultMyBenefitQuery(
...
export async function fetchMyBenefitDashboard(
  query: MyBenefitQuery,
  signal?: AbortSignal,
): Promise<MyBenefitResult> {
  ...
  const diagnostics: MyBenefitDiagnostics = {
...
export function createMyBenefitExportTask(query: MyBenefitQuery) {
  return createTask(query, MY_BENEFIT_EXPORT_ENDPOINT, 'mock-export', 'EXPORT-MY-BENEFIT-20260519-001')
}

export function createMyBenefitRenewTask(query: MyBenefitQuery) {
  return createTask(query, MY_BENEFIT_RENEW_ENDPOINT, 'mock-renew', 'RENEW-MY-BENEFIT-20260519-001')
}

export function createMyBenefitExpandTask(query: MyBenefitQuery, resource: MyBenefitResource) {
...
function createTask(query: MyBenefitQuery, endpoint: string, traceSuffix: string, taskId: string) {
  const diagnostics: MyBenefitDiagnostics = {
    endpoint,
...
function buildMyBenefitRequest(query: MyBenefitQuery) {
  return {
    campId: query.campId || DEFAULT_CAMP_ID,
  }
}

function readMyBenefitMockState(): MyBenefitMockState {
...
async function fetchMockMyBenefit(
  state: MyBenefitMockState,
  request: Record<string, unknown>,
  signal?: AbortSignal,
): Promise<ApiEnvelope<Omit<MyBenefitViewModel, 'provider' | 'state'>>> {
  ...
  const normalizedState: MyBenefitMockState = resources.length === 0 ? 'empty' : 'success'
...
async function fetchApiMyBenefit(
  _state: MyBenefitMockState,
...
function createViewModel(
  resources: MyBenefitResource[],
  records: MyBenefitRecord[],
): Omit<MyBenefitViewModel, 'provider' | 'state'> {
...
function adaptTargetResource(item: TargetResourceItem): MyBenefitResource {
...
function formatResourceFrom(item: TargetResourceItem) {
  if (item.goodsType === 2) {
...
function formatExpiration(expirationDate: number | null, isLongTerm: number | null) {
  if (isLongTerm === 1) return '无期限'
...
function createRecords(resources: MyBenefitResource[]): MyBenefitRecord[] {
...
function createServiceGroups(): MyBenefitServiceGroup[] {
  return [
    {
      id: 'accommodation',
      title: '专业住宿管理',
...
function writeDiagnostics(diagnostics: MyBenefitDiagnostics) {
...
function createMockTargetResponse(campId: string): TargetResourceResponse {
  return {
    success: true,
    errorCode: null,
    errorMsg: null,
...
function createBenefitResource(
  resourceName: string,
  expandQuotaDetailView: TargetResourceItem['resourceFrom']['expandQuotaDetailView'],
