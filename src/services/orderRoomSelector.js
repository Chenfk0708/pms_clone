...
export async function fetchOrderRoomSelectorOptions(campId, signal) {
    ...
    const [roomsPayload, categoriesPayload] = await Promise.all([
        postHudson(roomsEndpoint, { campId: normalizedCampId, pageNum: 1, pageSize: 999 }, signal),
        postHudson(roomCategoriesEndpoint, {
...
function adaptRoomSelectorGroups(roomsPayload, categoriesPayload) {
    ...
    const directGroups = readArray(asRecord(roomsPayload).roomCategoryRooms ??
