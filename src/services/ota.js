...
export async function saveMockOtaChannelSetup(form) {
    await delay(100);
    ...
    if (!channelExists)
        throw new Error(`未找到可配置的 OTA 渠道：${form.channelId}`);
    if (!form.ebookingAccount.trim())
        throw new Error('请填写 eBooking 账号');
    if (!form.storeName.trim() && !form.storeId.trim())
        throw new Error('请填写或选择本地门店');
...
function createPendingCatalogChannels(connectedIds = []) {
    return mockChannelCatalog
        .filter((channel) => !connectedIds.includes(channel.id))
        .map((channel) => createChannel(channel.id, channel.name, 'pending'));
}
function readMockDirectConnectedChannelIds() {
    if (typeof window === 'undefined')
        return [];
    try {
        ...
        return Array.from(new Set(parsed.map(String).map(normalizeChannelKey).filter((id) => catalogIds.has(id))));
    }
    catch {
        return [];
    }
}
function readMockChannelSetups() {
    if (typeof window === 'undefined')
        return {};
    try {
...
function writeMockChannelSetups(setups) {
    if (typeof window === 'undefined')
        return;
