...
export async function loadPaymentSettingPage(query = {}, signal) {
    const provider = resolveProvider();
    const requestBody = createListRequestBody(query);
    if (provider === 'api') {
        const payload = (await postJson(paymentWaysEndpoint, requestBody, signal));
...
export async function loadPaymentMethodDetail(methodId, signal) {
    const provider = resolveProvider();
    const requestBody = { methodId };
    if (provider === 'api') {
        const payload = (await postJson(paymentSettingDetailEndpoint, requestBody, signal));
...
export async function createPaymentMethod(input, signal) {
...
export async function updatePaymentMethodStatus(input, signal) {
    const requestBody = {
        campId: defaultCampId,
    ...
    return buildMutationResult(paymentSettingStatusEndpoint, requestBody, 'mock', `status-${input.nextStatus}-001`, `已${actionLabel}支付方式：${current.name}`);
}
export async function setDefaultPaymentMethod(input, signal) {
    const requestBody = {
        campId: defaultCampId,
        methodId: input.methodId,
    };
    ...
    return buildMutationResult(paymentSettingDefaultEndpoint, requestBody, 'mock', 'default-001', `已将${current.name}设为默认支付方式`);
}
export async function movePaymentMethod(input, signal) {
    const requestBody = {
        campId: defaultCampId,
        methodId: input.methodId,
    ...
    return buildMutationResult(paymentSettingSortEndpoint, requestBody, 'mock', `sort-${input.direction}-001`, input.direction === 'up' ? '已上移支付方式' : '已下移支付方式');
}
export async function createPaymentSettingExportTask(signal) {
    const requestBody = {
        campId: defaultCampId,
...
function createListRequestBody(query) {
    return {
        campId: query.campId || resolveCampId(),
        includeDisabled: true,
    };
}
function buildMockListEnvelope() {
...
function buildMockDetailEnvelope(methodId) {
...
function adaptPaymentWaysEnvelope(envelope) {
    if (envelope.code !== 0) {
        return {
            ...envelope,
            data: {
...
function adaptListEnvelope(envelope, requestBody, provider, mockState, endpoint = paymentSettingListEndpoint) {
    if (envelope.code !== 0) {
...
function adaptDetailEnvelope(envelope, requestBody, provider) {
    if (envelope.code !== 0) {
...
function buildMutationResult(endpoint, requestBody, provider, traceSuffix, message) {
    return {
        provider,
        endpoint,
...
function adaptSummary(method, index) {
    return {
        id: method.id,
        name: method.name,
...
function resolveProvider() {
    const configured = readRuntimeConfig('pms.paymentSettingProvider') || import.meta.env.VITE_PAYMENT_SETTING_PROVIDER;
...
function resolveMockState() {
    ...
    if (fromUrl)
        return fromUrl;
    const configured = readRuntimeConfig('pms.paymentSettingMockState') || import.meta.env.VITE_PAYMENT_SETTING_MOCK_STATE;
    if (configured === 'empty' || configured === 'error')
        return configured;
    return 'success';
}
function resolveMockLatencyMs() {
    const configured = readRuntimeConfig('pms.paymentSettingMockLatencyMs') || import.meta.env.VITE_PAYMENT_SETTING_MOCK_LATENCY_MS;
...
function normalizeMethodOrder(methods) {
...
function rebuildStatusScopedMethods(methods, status, scopedMethods) {
