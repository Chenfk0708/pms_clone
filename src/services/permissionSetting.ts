export type PermissionSettingProviderName = 'mock' | 'api'
export type PermissionSettingMockState = 'success' | 'empty' | 'error'

export type PermissionSettingRoleListQuery = {
  campId?: string
  keyword?: string
  pageNum?: number
  pageSize?: number
...
export type PermissionSettingRoleDetailQuery = {
  campId?: string
  roleId: string
}

export type PermissionSettingRoleSummary = {
  roleId: string
  roleName: string
  description: string
  memberCount: number
...
export type PermissionSettingPermissionRow = {
  moduleId: string
  moduleName: string
  permissions: string[]
}
...
export type PermissionSettingRoleListData = {
  provider: PermissionSettingProviderName
  mockState: PermissionSettingMockState
  endpoint: string
  traceId: string
  timestamp: string
  ...
  campId: string
  roles: PermissionSettingRoleSummary[]
  pagination: {
    page: number
    pageSize: number
    total: number
...
export type PermissionSettingRoleDetailData = {
  provider: PermissionSettingProviderName
  mockState: PermissionSettingMockState
  endpoint: string
  traceId: string
  ...
  requestSummary: string[]
  campId: string
  detail: PermissionSettingRoleDetail
}

export type PermissionSettingMutationData = {
  provider: PermissionSettingProviderName
  endpoint: string
  traceId: string
  timestamp: string
...
const basePermissionRows: PermissionSettingPermissionRow[] = [
...
const initialMockRoles: MockRoleRecord[] = [
  {
    id: 'role-admin',
    name: '管理员',
...
export async function loadPermissionSettingRoleList(
  query: PermissionSettingRoleListQuery,
  signal?: AbortSignal,
): Promise<PermissionSettingRoleListData> {
...
export async function loadPermissionSettingRoleDetail(
  query: PermissionSettingRoleDetailQuery,
  signal?: AbortSignal,
...
export async function createPermissionSettingRole(
  input: { campId?: string; roleName: string; description?: string },
  signal?: AbortSignal,
): Promise<PermissionSettingMutationData> {
  const provider = resolveProvider()
  if (provider === 'api') {
...
export async function renamePermissionSettingRole(
  input: { campId?: string; roleId: string; roleName: string; description?: string },
  signal?: AbortSignal,
): Promise<PermissionSettingMutationData> {
  const provider = resolveProvider()
  if (provider === 'api') {
...
export async function deletePermissionSettingRole(
  input: { campId?: string; roleId: string },
  signal?: AbortSignal,
): Promise<PermissionSettingMutationData> {
  const provider = resolveProvider()
  if (provider === 'api') {
...
function resolveMockState(): PermissionSettingMockState {
  ...
  const configured =
    readRuntimeConfig('pms.permissionSettingMockState') || import.meta.env.VITE_PERMISSION_SETTING_MOCK_STATE
  if (configured === 'empty' || configured === 'error') return configured
  return 'success'
}

function resolveDetailMockState(): PermissionSettingMockState {
...
async function loadRealRoleList(
  ...
  const requestBody = createRoleListRequestBody(query)
  const payload = await postHudson<RoleListPayload>(permissionRoleListEndpoint, requestBody, signal)
  return adaptRoleListPayload(
    payload,
    requestBody,
    'api',
    'success',
    'api-shezhi--qiye-shezhi--quanxian-shezhi-role-list',
    new Date().toISOString(),
  )
}

async function loadRealRoleDetail(
  query: PermissionSettingRoleDetailQuery,
  signal?: AbortSignal,
): Promise<PermissionSettingRoleDetailData> {
  const requestBody = createRoleDetailRequestBody(query)
  const payload = await postHudson<RoleDetailPayload>(permissionRoleDetailEndpoint, requestBody, signal)
...
function createRoleListRequestBody(query: PermissionSettingRoleListQuery) {
  return {
...
function createRoleDetailRequestBody(query: PermissionSettingRoleDetailQuery) {
  return {
    campId: query.campId || defaultPermissionSettingCampId,
    roleId: query.roleId,
  }
...
function buildMockRoleListEnvelope(
  query: PermissionSettingRoleListQuery,
): PermissionSettingEnvelope<RoleListPayload | null> {
  const requestBody = createRoleListRequestBody(query)
...
function buildMockRoleDetailEnvelope(
  query: PermissionSettingRoleDetailQuery,
): PermissionSettingEnvelope<RoleDetailPayload | null> {
...
function adaptRoleListEnvelope(
  envelope: PermissionSettingEnvelope<RoleListPayload | null>,
  requestBody: Record<string, unknown>,
  provider: PermissionSettingProviderName,
  mockState: PermissionSettingMockState,
): PermissionSettingRoleListData {
...
function adaptRoleListPayload(
  payload: RoleListPayload,
  requestBody: Record<string, unknown>,
  provider: PermissionSettingProviderName,
  mockState: PermissionSettingMockState,
  traceId: string,
...
function adaptRoleDetailEnvelope(
  envelope: PermissionSettingEnvelope<RoleDetailPayload | null>,
  requestBody: Record<string, unknown>,
  provider: PermissionSettingProviderName,
  mockState: PermissionSettingMockState,
): PermissionSettingRoleDetailData {
  if (envelope.code !== 0 || !envelope.data) {
...
function adaptRoleDetailPayload(
  payload: RoleDetailPayload,
  requestBody: Record<string, unknown>,
  provider: PermissionSettingProviderName,
  mockState: PermissionSettingMockState,
  traceId: string,
...
function adaptRoleItem(value: unknown): PermissionSettingRoleSummary {
...
function adaptPermissionRow(value: unknown): PermissionSettingPermissionRow {
  const record = asRecord(value)
...
function createInitialMockDetails(roles: MockRoleRecord[]) {
  return Object.fromEntries(
    roles.map((role) => [
      ...
      } satisfies MockDetailRecord,
    ]),
  ) as Record<string, MockDetailRecord>
}

function buildRolePermissionRows(roleId: string) {
  if (roleId === 'role-investor') {
    return clonePermissionRows(
      basePermissionRows.map((row) =>
        ['statistics', 'ledger', 'wallet', 'notify-order', 'benefit'].includes(row.moduleId)
          ? row
...
function cloneRoles(roles: MockRoleRecord[]) {
