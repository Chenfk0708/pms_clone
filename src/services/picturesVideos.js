...
const REQUEST_ENDPOINT = '/medias/page/get';
...
export function defaultPicturesVideosRequest(state = resolvePicturesVideosMockState()) {
    return {
        campId: DEFAULT_CAMP_ID,
        pageNum: 1,
        pageSize: 50,
...
export function resolvePicturesVideosMockState(search = currentSearch()) {
    ...
    const configured = params.get('picturesVideosMockState') ||
...
export async function fetchPicturesVideosView(request, signal) {
    ...
    if (provider === 'api') {
        throw new PicturesVideosServiceError('图片视频数据加载失败，请稍后重试', envelope(503, 'service unavailable', null, 'api-setting-pictures-videos-unavailable'), normalizedRequest);
    }
    await waitForMockLatency(signal);
    if (normalizedRequest.state === 'error') {
        throw new PicturesVideosServiceError('图片视频数据加载失败，请稍后重试', envelope(50031, 'pictures and videos query failed', null, 'mock-setting-pictures-videos-error-001'), normalizedRequest);
    }
...
function toContractRequest(request) {
    return {
...
function recordLastRequest(provider, state, request) {
    if (typeof window === 'undefined')
        return;
    window.localStorage.setItem(LAST_REQUEST_STORAGE_KEY, JSON.stringify({
        provider,
        state,
        endpoint: REQUEST_ENDPOINT,
        method: 'POST',
