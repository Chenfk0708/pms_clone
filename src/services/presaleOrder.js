...
export function createPresaleOrderRequestBody(filters, campId) {
    const body = {
        campId,
...
export function getPresaleOrderProviderName() {
    const fromStorage = typeof window === 'undefined' ? null : window.localStorage.getItem(PRESALE_PROVIDER_STORAGE_KEY);
    if (fromStorage === 'real' || fromStorage === 'mock')
        return fromStorage;
    const fromEnv = import.meta.env.VITE_PRESALE_ORDER_PROVIDER;
    return fromEnv === 'real' ? 'real' : 'mock';
}
export function normalizePresaleOrderMockState(value) {
    return value === 'empty' || value === 'error' ? value : 'success';
}
export async function loadPresaleOrderData(filters, signal) {
    ...
    try {
        const response = providerName === 'mock'
...
async function postPresaleOrderMock(filters) {
    const params = new URLSearchParams(window.location.search);
    const state = normalizePresaleOrderMockState(params.get('mockState') || window.localStorage.getItem(PRESALE_MOCK_STATE_STORAGE_KEY));
    if (state === 'error') {
        return createPresaleOrderResponse(null, `${TASK_TRACE_PREFIX}-list-error`, 500, '预售券订单加载失败');
    }
...
async function postPresaleOrderReal(filters, signal) {
    let campId = filters.campId;
    if (!campId)
        campId = await fetchDefaultCampId(signal);
    if (!campId)
        throw new Error('未取得当前门店，无法加载预售券订单');
    ...
    return createPresaleOrderResponse(listData, 'real-dingdan--yushouquan-dingdan--yushouquan-dingdan-list-001');
}
function createPresaleOrderResponse(data, traceId, code = 0, message = 'success') {
    return {
        code,
        message,
        data,
...
function unwrapPresaleOrderResponse(response) {
    if (response.code !== 0)
        throw new Error(response.message || '预售券订单加载失败');
    if (response.data === null || response.data === undefined)
        throw new Error('预售券订单加载失败');
    return response.data;
}
function adaptListData(data, context) {
...
async function fetchSourceOptions(campId, signal) {
...
async function fetchPaymentOptions(campId, signal) {
    const payload = await postHudson(PRESALE_PAYMENT_TYPES_ENDPOINT, { campId, bizTypes: [3], isEnable: 1 }, signal);
...
async function fetchCategoryOptions(signal) {
...
function adaptHudsonOrderPayload(payload, context) {
...
function adaptOrderRow(value, index) {
...
function filterRows(rows, filters) {
    ...
    return rows.filter((row) => {
        if (filters.orderState !== '0' && row.orderStateValue !== filters.orderState)
            return false;
        if (filters.productType && row.productTypeValue !== filters.productType)
            return false;
        if (filters.source && row.sourceId !== filters.source)
            return false;
        if (filters.category && row.categoryId !== filters.category)
            return false;
        if (filters.payment && row.paymentId !== filters.payment)
            return false;
        if (filters.afterSale && row.afterSaleValue !== filters.afterSale)
            return false;
        if (keyword && !`${row.id} ${row.buyer} ${row.contact} ${row.productName}`.includes(keyword))
            return false;
        return true;
    });
}
