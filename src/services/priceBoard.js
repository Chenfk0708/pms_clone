...
export async function loadPriceBoardData(signal) {
    if (resolvePriceBoardProviderName() === 'mock') {
        return loadMockPriceBoardData();
    }
    const campInfo = readCampInfo(await postHudson('/camps/get', {}, signal));
    ...
    const [editionResources, productPage, paymentTypes] = await Promise.all([
        ...
        postHudson('/weiRoomCategories/page/get', {
            campId: PRICE_BOARD_PRODUCT_CAMP_ID,
            buyCampId: campId,
            roomCategoryTypes: [1],
            goodsTypes: [7],
        }, signal),
        postHudson('/paymentTypes/get/v2', { campId, bizTypes: [3], isEnable: 1 }, signal),
    ]);
...
function loadMockPriceBoardData() {
    const mode = resolvePriceBoardMockMode();
    const envelope = mode === 'error'
...
function mockPriceBoardSuccessEnvelope() {
    return {
        code: 0,
        message: 'success',
...
function mockPriceBoardEmptyEnvelope() {
    return {
        code: 0,
        message: 'success',
...
function mockPriceBoardErrorEnvelope() {
    return {
        code: 50001,
...
function resolvePriceBoardProviderName() {
...
function resolvePriceBoardMockMode() {
...
function findPriceBoardProduct(data) {
...
function normalizeDurationOptions(product) {
...
function normalizePaymentTypes(data) {
