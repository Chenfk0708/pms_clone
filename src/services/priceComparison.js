...
export function normalizePriceComparisonMockState(value) {
...
function buildDashboardData(filters, mockState) {
...
async function loadMockDashboard(request) {
...
function adaptDashboardResponse(response, provider) {
    if (response.code !== 0) {
...
export async function loadPriceComparisonDashboard(request = {}) {
    const provider = request.provider ?? getConfiguredProvider();
    const response = provider === 'mock'
