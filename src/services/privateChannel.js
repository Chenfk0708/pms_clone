...
export function loadPrivateChannel(query = {}) {
...
export function createMockPrivateChannelResponse(scenario) {
    if (scenario === 'error') {
        return {
            code: 50301,
            message: '私域渠道数据加载失败',
...
function adaptPrivateChannelResponse(response, provider, scenario) {
    return {
        ...response.data,
        contract: {
            provider,
