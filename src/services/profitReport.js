...
export function getDefaultProfitReportFilters() {
...
export function createProfitReportRequestBody(filters) {
    return {
...
function createProfitReportExportRequestBody(filters) {
    return {
...
async function fetchMockDashboard(filters) {
    ...
    if (mockState === 'error') {
        return createEnvelope(5001, '利润报表数据加载失败，请稍后重试', createEmptyPayload(filters.pageNum, filters.pageSize), 'error');
    }
    const rows = mockState === 'empty' ? [] : selectRows(filters);
    const total = mockState === 'empty' ? 0 : 32;
    return createEnvelope(0, 'success', {
        rows,
        pagination: {
...
function createEmptyPayload(page, pageSize) {
    return {
        rows: [],
...
function createEnvelope(code, message, data, suffix) {
    return {
        code,
        message,
        data,
...
function adaptMockDashboard(envelope, filters) {
    if (envelope.code !== 0) {
...
async function fetchApiDashboard(filters, signal) {
    ...
    const [storesPayload, roomCategoriesPayload, channelsPayload, roomGroupsPayload, reportPayload] = await Promise.all([
        postHudson(profitReportStoreEndpoint, { campId: filters.campId, pageSize: 999, pageNum: 1, channelId: 0, isAvailability: '1' }, signal),
        postHudson(profitReportRoomCategoryEndpoint, { campId: filters.campId, pageNum: 1, pageSize: 9999, isAvailability: 1, channelId: 0 }, signal),
        postHudson(profitReportChannelEndpoint, { campId: filters.campId }, signal),
        postHudson(profitReportRoomGroupEndpoint, { campId: filters.campId }, signal),
        postHudson(profitReportEndpoint, requestBody, signal),
    ]);
...
function adaptStoreOptions(payload) {
    ...
    return [{ id: 'all', label: '全部门店' }].concat(list.map((item, index) => ({
        id: String(item.poiId ?? `poi-${index}`),
        label: String(item.poiName ?? `门店 ${index + 1}`),
    })));
}
function adaptRoomCategoryOptions(payload) {
    const list = Array.isArray(payload.list) ? payload.list.map(asRecord) : [];
...
function adaptChannelOptions(payload) {
...
function adaptRoomGroupOptions(payload) {
...
function createRow(date, roomFeeMinusCommission, ticketPrice, cateringPrice, otherOrderExpense, writeDownIncome, totalIncome, writeDownExpenses, profitPrice, isTotal = false) {
    return {
        date,
        isTotal,
...
function calculateRate(profit, income) {
    if (income <= 0) {
        return '0.00%';
    }
