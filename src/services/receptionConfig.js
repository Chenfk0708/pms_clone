...
export function resolveReceptionConfigRuntimeConfig(location) {
...
export async function fetchReceptionConfigDashboard(query, signal) {
...
export function createReceptionConfigSaveTask(query) {
    const diagnostics = {
...
export function createReceptionConfigExportTask(query) {
    const diagnostics = {
...
function buildReceptionConfigRequest(query) {
    return {
...
function readReceptionConfigMockState() {
    const configured = readRuntimeConfig('pms.receptionConfigMockState') || import.meta.env.VITE_RECEPTION_CONFIG_MOCK_STATE;
    if (configured === 'empty' || configured === 'error')
        return configured;
    return 'success';
}
async function fetchMockReceptionConfig(state, query, _request, signal) {
    ...
    const normalizedState = rules.length === 0 && staffMembers.length === 0 ? 'empty' : 'success';
    return {
        code: 0,
        message: 'success',
...
async function fetchApiReceptionConfig(state, _request, signal) {
    await delay(80, signal);
    return {
        code: 50002,
        message: '接待配置数据加载失败，请重试',
...
function createBackendData(rules, staffMembers, state) {
...
function filterRules(rules, query) {
    ...
    return rules.filter((rule) => {
        if (query.staffGroup && rule.staffGroup !== query.staffGroup)
            return false;
        if (query.configStatus && rule.status !== query.configStatus)
            return false;
        if (!keyword)
            return true;
...
function filterStaffMembers(staffMembers, query) {
    const keyword = query.keyword.trim().toLowerCase();
    return staffMembers.filter((staffMember) => {
        if (query.staffGroup && staffMember.staffGroup !== query.staffGroup)
            return false;
        if (!keyword)
            return true;
...
const defaultPreviewMessage = '欢迎入住天麓会宿公寓，已为您安排夜班接待专员在线值守，如需停车、入住指引或延迟退房，请直接回复当前会话。';
const mockRules = [
    {
        id: 'rule-001',
        name: '新客入住欢迎规则',
