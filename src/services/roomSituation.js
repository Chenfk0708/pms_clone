...
export function resolveRoomSituationCampId() {
    ...
    const fromEnv = import.meta.env.VITE_PMS_CAMP_ID?.trim();
...
export function resolveRoomSituationProvider() {
    const params = new URLSearchParams(window.location.search);
    const rawProvider = params.get('roomSituationProvider') ??
        window.localStorage.getItem(roomSituationProviderConfigKey) ??
        import.meta.env.VITE_ROOM_SITUATION_PROVIDER ??
        'mock';
    return rawProvider === 'real' ? 'real' : 'mock';
}
export function formatRoomSituationDataSource(endpoint) {
    void endpoint;
    return '房情表数据已同步';
}
export function formatRoomSituationFeedback(state) {
...
export async function fetchRoomSituationStores(campId, signal) {
    const requestBody = { campId, pageSize: 999, pageNum: 1, channelId: 0, isAvailability: '1' };
    const data = resolveRoomSituationProvider() === 'mock'
        ? unwrapRoomSituationResponse(mockRoomSituationStores(requestBody))
        : await postHudson(roomSituationStoresEndpoint, requestBody, signal);
...
export async function fetchDailyRoomSituation(filters, signal) {
    const requestBody = {
    ...
    const data = resolveRoomSituationProvider() === 'mock'
        ...
        : await postHudson(dailyRoomSituationEndpoint, requestBody, signal);
    const list = Array.isArray(data.list) ? data.list.map(asRecord) : [];
    return {
...
export async function fetchForwardRoomSituation(filters, signal) {
    const requestBody = {
        campId: filters.campId,
    ...
    const data = resolveRoomSituationProvider() === 'mock'
        ...
        : await postHudson(forwardRoomSituationEndpoint, requestBody, signal);
    const list = Array.isArray(data.list) ? data.list.map(asRecord) : [];
    return {
...
function mockRoomSituationStores(requestBody) {
    return createRoomSituationResponse('stores', {
        total: 1,
        pageNum: 1,
...
function mockDailyRoomSituation(requestBody) {
...
function mockForwardRoomSituation(requestBody) {
    const scenario = resolveRoomSituationMockScenario();
...
function createRoomSituationResponse(name, data, requestBody, code = 0, message = 'success') {
    return {
        code,
        message,
        data,
...
function unwrapRoomSituationResponse(response) {
    if (response.code !== 0) {
        throw new Error(`${response.message}（traceId: ${response.traceId}）`);
    }
    return response.data;
}
function resolveRoomSituationMockScenario() {
    const params = new URLSearchParams(window.location.search);
...
function adaptDailyRow(row, index) {
    return {
...
function adaptForwardRow(row, index) {
...
function buildForwardMockRows(requestBody) {
...
function normalizeMockDate(value) {
