...
export type ScrmMemberLevelProviderName = 'mock' | 'api'
export type ScrmMemberLevelMockState = 'success' | 'empty' | 'error'
export type ScrmMemberLevelStatus = 'enabled' | 'disabled'

export type ScrmMemberLevelFilters = {
  storeId: string
  status: string
  keyword: string
  page: number
  pageSize: number
  mockState: ScrmMemberLevelMockState
}

export type ScrmMemberLevelOption = {
  value: string
  label: string
}

export type ScrmMemberLevelMetric = {
  key: string
  label: string
  value: string
  description: string
...
export type ScrmMemberLevel = {
  ...
  benefits: string
  cardColor: string
  status: ScrmMemberLevelStatus
  memberCount: number
  updatedAt: string
}

export type ScrmMemberUpgradeRule = {
  id: string
  label: string
  selected: boolean
}

export type ScrmMemberLevelDashboard = {
  filters: ScrmMemberLevelFilters
  stores: ScrmMemberLevelOption[]
  statusOptions: ScrmMemberLevelOption[]
  metrics: ScrmMemberLevelMetric[]
  levels: ScrmMemberLevel[]
  upgradeRules: ScrmMemberUpgradeRule[]
  pagination: {
    page: number
    pageSize: number
    total: number
  ...
  requestedAt: string
  provider: ScrmMemberLevelProviderName
  traceId: string
}

export type ScrmMemberLevelSaveInput = {
  id?: string
  name: string
  rank: number
  roomDiscount: string
...
type ScrmMemberLevelPayload = Omit<ScrmMemberLevelDashboard, 'filters' | 'provider' | 'traceId'>

export function createDefaultScrmMemberLevelFilters(searchParams = new URLSearchParams()): ScrmMemberLevelFilters {
  return {
...
export async function fetchScrmMemberLevelDashboard(
  filters: ScrmMemberLevelFilters,
  providerName = getScrmMemberLevelProviderName(),
): Promise<ScrmMemberLevelDashboard> {
...
export async function saveScrmMemberLevel(input: ScrmMemberLevelSaveInput) {
  await delay(80)
  if (!input.name.trim()) {
...
export async function saveScrmMemberUpgradeRule(ruleId: string) {
  await delay(80)
  if (!ruleId) {
...
export function buildScrmMemberLevelRequest(filters: ScrmMemberLevelFilters) {
  return {
...
async function fetchMockDashboard(
  filters: ScrmMemberLevelFilters,
): Promise<UnifiedEnvelope<ScrmMemberLevelPayload>> {
  await delay(120)
...
function adaptEnvelope(
  envelope: UnifiedEnvelope<ScrmMemberLevelPayload>,
  filters: ScrmMemberLevelFilters,
  provider: ScrmMemberLevelProviderName,
): ScrmMemberLevelDashboard {
  if (envelope.code !== 0) {
...
function createPayload(filters: ScrmMemberLevelFilters, levels: ScrmMemberLevel[]): ScrmMemberLevelPayload {
...
function filterLevels(levels: ScrmMemberLevel[], filters: ScrmMemberLevelFilters) {
...
function validateFilters(filters: ScrmMemberLevelFilters) {
...
function toMockState(value: string | null): ScrmMemberLevelMockState {
...
const mockLevels: ScrmMemberLevel[] = [
  {
