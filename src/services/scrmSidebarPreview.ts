export type ScrmSidebarProviderMode = 'mock' | 'api'
export type ScrmSidebarScenario = 'success' | 'empty' | 'error'

export type ScrmSidebarFilters = {
  campId: string
  poiId: string
  date: string
  channel: string
  ...
  page: number
  pageSize: number
  scenario?: ScrmSidebarScenario
}

export type ScrmLookupOption = {
  id: string
  label: string
}

export type ScrmMetric = {
  id: string
  label: string
  value: string
  change: string
...
export type ScrmReplyTemplate = {
  id: string
  title: string
  content: string
}

export type ScrmRoomSuggestion = {
  id: string
  roomName: string
  status: string
...
export type ScrmTrendPoint = {
  label: string
  sessions: number
  orders: number
}

export type ScrmSidebarDashboard = {
  providerMode: ScrmSidebarProviderMode
  endpoint: string
  requestBody: Record<string, unknown>
  stores: ScrmLookupOption[]
  channels: ScrmLookupOption[]
  metrics: ScrmMetric[]
  conversations: ScrmConversation[]
  pendingItems: ScrmPendingItem[]
  replyTemplates: ScrmReplyTemplate[]
  roomSuggestions: ScrmRoomSuggestion[]
  trend: ScrmTrendPoint[]
  pagination: {
    page: number
    pageSize: number
    total: number
...
type RawDashboardData = Omit<
  ScrmSidebarDashboard,
  'providerMode' | 'endpoint' | 'requestBody' | 'traceId' | 'updatedAt'
>

type ScrmSidebarProvider = {
  fetchDashboard(filters: ScrmSidebarFilters): Promise<ScrmSidebarDashboard>
}
...
const stores: ScrmLookupOption[] = [
  { id: 'ALL', label: '全部门店' },
  { id: 'qianhai', label: '天落会宿公寓(前海壹方城宝安中心店)' },
]

const channels: ScrmLookupOption[] = [
  { id: 'ALL', label: '全部渠道' },
  { id: 'tujia', label: '途家' },
  { id: 'ctrip', label: '携程民宿' },
...
const conversations: ScrmConversation[] = [
  {
    id: 'conv-001',
...
const replyTemplates: ScrmReplyTemplate[] = [
  {
    id: 'reply-renew',
    title: '续住引导',
...
const roomSuggestions: ScrmRoomSuggestion[] = [
...
const pendingItems: ScrmPendingItem[] = [
...
export function createScrmSidebarRequestBody(filters: ScrmSidebarFilters): Record<string, unknown> {
  return {
...
function getScrmSidebarProvider(mode: ScrmSidebarProviderMode): ScrmSidebarProvider {
  if (mode === 'api') return apiScrmSidebarProvider
  return mockScrmSidebarProvider
}

const mockScrmSidebarProvider: ScrmSidebarProvider = {
...
const apiScrmSidebarProvider: ScrmSidebarProvider = {
  async fetchDashboard(filters) {
    const requestBody = createScrmSidebarRequestBody(filters)
...
function filterConversations(filters: ScrmSidebarFilters) {
  const keyword = filters.keyword.trim()
  return conversations.filter((item) => {
    if (filters.channel !== 'ALL' && item.channel !== filters.channel) return false
    if (!keyword) return true
    return [item.guestName, item.roomName, item.lastMessage, item.orderNo, item.preference].some((value) =>
      value.includes(keyword),
    )
  })
}

function createMetrics(list: ScrmConversation[]): ScrmMetric[] {
...
function createEnvelope(traceId: string, data: RawDashboardData): ApiEnvelope<RawDashboardData> {
  return {
    code: 0,
    message: 'success',
    data,
...
function adaptConversation(item: ScrmConversation, index: number): ScrmConversation {
  return {
...
function normalizeOptions(options: unknown): ScrmLookupOption[] {
