...
export function createDefaultShiftRecordFilters(searchParams = new URLSearchParams()) {
    return {
...
export function buildShiftRecordRequest(filters) {
    return {
...
export async function fetchShiftRecordDashboard(filters, providerName = getShiftRecordProviderName(), signal) {
...
export async function exportShiftRecords(filters, providerName = getShiftRecordProviderName(), signal) {
    validateFilters(filters);
    if (providerName === 'api') {
...
function getShiftRecordProviderName() {
    if (typeof window === 'undefined')
        return 'mock';
    const configured = window.localStorage.getItem(SHIFT_RECORD_PROVIDER_KEY)?.trim();
    return configured === 'api' || configured === 'real' ? 'api' : 'mock';
}
async function fetchMockShiftRecordDashboard(filters, signal) {
...
function adaptShiftRecordEnvelope(envelope, filters, provider) {
    if (envelope.code !== 0) {
...
function createEmptyPayload(filters) {
    return {
        stores: storeOptions,
        employees: employeeOptions,
        rows: [],
...
function filterRows(filters) {
    return mockRows.filter((row) => {
        if (filters.storeId !== 'all' && row.storeId !== filters.storeId)
            return false;
        if (filters.handoverUserId !== 'all' && row.handoverUserId !== filters.handoverUserId)
            return false;
        if (filters.receiverUserId !== 'all' && row.receiverUserId !== filters.receiverUserId)
            return false;
        if (filters.startDate && row.handoverDate < filters.startDate)
            return false;
        if (filters.endDate && row.handoverDate > filters.endDate)
            return false;
        return true;
    });
}
async function fetchApiShiftRecordDashboard(filters, signal) {
    const [listPayload, storePayload, employeePayload] = await Promise.all([
        postHudson(SHIFT_RECORD_LIST_PATH, buildListRequest(filters), signal),
        postHudson(SHIFT_RECORD_STORE_PATH, buildStoreRequest(filters), signal),
        postHudson(SHIFT_RECORD_EMPLOYEE_PATH, { campId: filters.campId }, signal),
    ]);
...
function buildListRequest(filters) {
    return {
        campId: filters.campId,
        pageNum: filters.pageNum,
        pageSize: filters.pageSize,
...
function buildStoreRequest(filters) {
    return {
        campId: filters.campId,
        pageSize: 999,
        pageNum: 1,
...
function adaptEmployeeOptions(payload) {
...
function resolveShiftRecordStatusLabel(workStatus, fallback) {
    if (workStatus === 0)
        return '待交班';
    if (workStatus === 1)
        return '已完成';
...
function parseWorkReport(value) {
    if (!value)
        return null;
    try {
        return normalizeWorkReport(JSON.parse(value));
    }
    catch {
        return null;
    }
}
function normalizeWorkReport(payload) {
    return {
