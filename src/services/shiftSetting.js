...
export async function saveShiftConfigs(filters, drafts, providerName = getShiftSettingProviderName(), signal) {
    ...
    if (providerName === 'api') {
        const payload = await postHudson(SHIFT_SETTING_CONFIG_SAVE_PATH, { campId: filters.campId, drafts }, signal);
...
export async function saveShiftGoods(filters, drafts, providerName = getShiftSettingProviderName(), signal) {
    ...
    if (providerName === 'api') {
        const payload = await postHudson(SHIFT_SETTING_GOODS_SAVE_PATH, { campId: filters.campId, drafts }, signal);
        const dashboard = await fetchApiShiftSettingDashboard(filters, signal);
        return {
            provider: 'api',
...
function getShiftSettingProviderName() {
    ...
    const configured = params.get('provider') ||
        ...
        import.meta.env.VITE_SHIFT_SETTING_PROVIDER ||
        'api';
