...
export type SmartDoorLockProviderName = 'mock' | 'api'
export type SmartDoorLockMockState = 'success' | 'empty' | 'error'
export type SmartDoorLockTabKey = 'password' | 'card'
export type SmartDoorLockBrandAction = 'login' | 'activate' | 'overview'
export type SmartDoorLockAccountTone = 'connected' | 'attention'
export type SmartDoorLockViewState = 'ready' | 'empty'

export type SmartDoorLockQuery = {
  campId: string
  tab: SmartDoorLockTabKey
  mockState: SmartDoorLockMockState
}

export type SmartDoorLockBrand = {
  id: string
  tab: SmartDoorLockTabKey
  label: string
  logo: string
  tone: 'blue' | 'cyan' | 'black' | 'dark' | 'sky' | 'red'
  action: SmartDoorLockBrandAction
  accountLabel?: string
  passwordLabel?: string
}

export type SmartDoorLockAccount = {
  id: string
  tab: SmartDoorLockTabKey
  brandId: string
  brandLabel: string
  ...
  statusLabel: string
  statusTone: SmartDoorLockAccountTone
  syncDisabled: boolean
  syncDisabledReason?: string
  linkedBy: string
}

export type SmartDoorLockTabView = {
  key: SmartDoorLockTabKey
  label: string
  leadText: string
  accountsTitle: string
  accounts: SmartDoorLockAccount[]
  brandsTitle: string
  brands: SmartDoorLockBrand[]
}

export type SmartDoorLockDashboard = {
  provider: SmartDoorLockProviderName
  traceId: string
  requestedAt: string
  requestedAtLabel: string
  activeTab: SmartDoorLockTabKey
  state: SmartDoorLockViewState
  tabs: SmartDoorLockTabView[]
  cardSystemEnabled: boolean
  cardSystemSummary: {
    title: string
    description: string
...
export type SmartDoorLockLoginPayload = {
  brandId: string
  tab: SmartDoorLockTabKey
  userName: string
  password: string
...
type SmartDoorLockDashboardPayload = Omit<SmartDoorLockDashboard, 'provider' | 'traceId'>
type SmartDoorLockAccountPayload = Omit<SmartDoorLockAccount, 'id'>
...
const passwordBrands: SmartDoorLockBrand[] = [
...
const cardBrands: SmartDoorLockBrand[] = [
...
const successPasswordAccounts: SmartDoorLockAccount[] = [
  {
...
export function createDefaultSmartDoorLockQuery(searchParams = new URLSearchParams()): SmartDoorLockQuery {
  return {
...
export async function fetchSmartDoorLockDashboard(
  query: SmartDoorLockQuery,
  signal?: AbortSignal,
  providerName = getSmartDoorLockProviderName(),
): Promise<SmartDoorLockDashboard> {
  validateQuery(query)
  if (providerName === 'api') {
...
export async function submitSmartDoorLockAccount(
  payload: SmartDoorLockLoginPayload,
  signal?: AbortSignal,
  providerName = getSmartDoorLockProviderName(),
): Promise<SmartDoorLockAccount> {
...
function createDashboardPayload(activeTab: SmartDoorLockTabKey, state: SmartDoorLockViewState): SmartDoorLockDashboardPayload {
  return {
...
function buildAccountEnvelope(payload: SmartDoorLockLoginPayload): UnifiedEnvelope<SmartDoorLockAccountPayload> {
...
function createSubmittedAccountPayload(
  brand: SmartDoorLockBrand,
  payload: SmartDoorLockLoginPayload,
): SmartDoorLockAccountPayload {
  return {
    tab: payload.tab,
    brandId: brand.id,
...
function adaptDashboardEnvelope(
  envelope: UnifiedEnvelope<SmartDoorLockDashboardPayload>,
  provider: SmartDoorLockProviderName,
): SmartDoorLockDashboard {
  if (envelope.code !== 0) {
...
function adaptAccountEnvelope(envelope: UnifiedEnvelope<SmartDoorLockAccountPayload>): SmartDoorLockAccount {
  if (envelope.code !== 0) {
...
function validateQuery(query: SmartDoorLockQuery) {
  if (!query.campId.trim()) {
    throw new Error(text.missingCamp)
  }
}

function validateLoginPayload(payload: SmartDoorLockLoginPayload) {
  if (!payload.brandId.trim()) throw new Error(text.missingBrand)
  if (!payload.userName.trim()) throw new Error(text.missingUserName)
  if (!payload.password.trim()) throw new Error(text.missingPassword)
}

function toMockState(value: string | null): SmartDoorLockMockState {
  return value === 'empty' || value === 'error' ? value : 'success'
}

function toTab(value: string | null): SmartDoorLockTabKey {
