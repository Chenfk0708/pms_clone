...
export type SmartHotelGlobalSettingProviderName = 'mock' | 'api'
export type SmartHotelGlobalSettingMockState = 'success' | 'empty' | 'error'
export type SmartHotelGlobalSettingTabId = 'rules' | 'guide' | 'wifi'

export type SmartHotelGlobalSettingFilters = {
  campId: string
  mockState: SmartHotelGlobalSettingMockState
}

export type SmartHotelGlobalSettingToggle = {
  id: string
  label: string
  checked: boolean
  disabled: boolean
...
export type SmartHotelGlobalSettingChoice = {
  id: string
  title: string
  description?: string
  selected?: boolean
...
export type SmartHotelGlobalSettingGuideField = {
  id: string
  label: string
  value: string
}

export type SmartHotelGlobalSettingSmsTemplate = {
  id: string
  title: string
  content: string
}

export type SmartHotelGlobalSettingDashboard = {
  provider: SmartHotelGlobalSettingProviderName
  state: SmartHotelGlobalSettingMockState
  traceId: string
  timestamp: string
  requestBody: {
    campId: string
  ...
  pageTitle: string
  versionLabel: string
  tabs: Array<{ id: SmartHotelGlobalSettingTabId; label: string }>
  alertText: string
  ...
  identitySummary: {
    realNameBalance: string
    smsBalance: string
    channelName: string
  }
  paymentMethods: string[]
  smsTemplates: SmartHotelGlobalSettingSmsTemplate[]
  flowSteps: string[]
  ...
  toggles: {
    autoInvite: SmartHotelGlobalSettingToggle
    deposit: SmartHotelGlobalSettingToggle
    guestStatus: SmartHotelGlobalSettingToggle
    dirtyRoomBlock: SmartHotelGlobalSettingToggle
    earlyPassword: SmartHotelGlobalSettingToggle
  }
  guideFields: SmartHotelGlobalSettingGuideField[]
  wifiFields: SmartHotelGlobalSettingGuideField[]
  emptyState?: {
    title: string
    description: string
    actionLabel: string
...
type SmartHotelGlobalSettingPayload = Omit<
  SmartHotelGlobalSettingDashboard,
  'provider' | 'state' | 'traceId' | 'timestamp'
>
...
type SystemConfigApiItem = {
  configKey?: string
  configValue?: unknown
}

type SystemConfigsApiResponse = {
  configs?: SystemConfigApiItem[]
}

type SmsAccountApiResponse = {
  curSmsCount?: string
}

type DepositChannelApiItem = {
  channelName?: string
  isOpen?: number
}

type DepositChannelApiResponse = {
  select?: DepositChannelApiItem[]
}

type SmsTemplateApiItem = {
  smsTemplateMsgConfigId?: string
  name?: string
  passContent?: string
}

type SmsTemplatePageApiResponse = {
  total?: number
  list?: SmsTemplateApiItem[]
}

type RoomCategoryProductInfoApiItem = {
  earliestCheckInTime?: number
  latestCheckOutTime?: number
}

type RoomCategoryApiItem = {
  roomCategoryId?: string
  roomCategoryProductInfoViews?: RoomCategoryProductInfoApiItem[]
  earliestCheckInTime?: number
  latestCheckOutTime?: number
}

type RoomCategoryPageApiResponse = {
  total?: number
  list?: RoomCategoryApiItem[]
}

type WeiRoomCategoryPageApiResponse = {
  total?: number
}
...
type PaymentWayApiItem = {
  paymentWayName?: string
  isEnable?: number
}

type PaymentWayApiResponse = {
  paymentWays?: PaymentWayApiItem[]
}

type SystemConfigDetailApiResponse = {
  configValue?: unknown
}
...
export function createDefaultSmartHotelGlobalSettingFilters(
  searchParams = new URLSearchParams(),
): SmartHotelGlobalSettingFilters {
...
export async function fetchSmartHotelGlobalSettingDashboard(
  filters: SmartHotelGlobalSettingFilters,
  signal?: AbortSignal,
  providerName = getSmartHotelGlobalSettingProviderName(),
): Promise<SmartHotelGlobalSettingDashboard> {
...
function getSmartHotelGlobalSettingProviderName(): SmartHotelGlobalSettingProviderName {
...
function buildMockEnvelope(
  filters: SmartHotelGlobalSettingFilters,
): UnifiedEnvelope<SmartHotelGlobalSettingPayload> {
...
function createPayload(): SmartHotelGlobalSettingPayload {
  return {
    saveEnabled: false,
    pageTitle: '全局设置',
...
function adaptEnvelope(
  envelope: UnifiedEnvelope<SmartHotelGlobalSettingPayload>,
  provider: SmartHotelGlobalSettingProviderName,
  state: SmartHotelGlobalSettingMockState,
): SmartHotelGlobalSettingDashboard {
  if (envelope.code !== 0) {
...
async function fetchApiSmartHotelGlobalSettingDashboard(
  filters: SmartHotelGlobalSettingFilters,
  signal?: AbortSignal,
): Promise<SmartHotelGlobalSettingDashboard> {
  ...
  const [
    ...
    postHudsonEnvelope<SystemConfigsApiResponse>('/systemConfigs/get', { campId: filters.campId }, signal),
    postHudsonEnvelope<SmsAccountApiResponse>('/smsAccount/get', { campId: filters.campId }, signal),
    postHudsonEnvelope<DepositChannelApiResponse>('/select/calChannel4Deposit/get', { campId: filters.campId }, signal),
    postHudsonEnvelope<SmsTemplatePageApiResponse>(
      '/smsTemplateMsgConfig/page/get',
      { campId: filters.campId, sendType: 0, pageNum: 1, pageSize: 100 },
      signal,
    ),
    postHudsonEnvelope<RoomCategoryPageApiResponse>(
      '/roomCategories/page/get',
      {
    ...
    postHudsonEnvelope<WeiRoomCategoryPageApiResponse>(
      '/weiRoomCategories/page/get',
      {
    ...
    postHudsonEnvelope<PaymentTypeApiResponse>(
      '/paymentTypes/get/v2',
      { campId: filters.campId, bizTypes: [1], isEnable: 1 },
      signal,
    ),
    postHudsonEnvelope<PaymentWayApiResponse>('/paymentWays/get', { campId: filters.campId }, signal),
    postHudsonEnvelope<SystemConfigDetailApiResponse>(
      '/systemConfig/checkInGuideShowStrategy/get',
      { campId: filters.campId },
      signal,
    ),
  ...
  const guestStatusChecked = readBooleanConfig(
    systemConfigs.data?.configs,
    'hudson.basic.isAutoCheckInAndCheckOut',
    basePayload.toggles.guestStatus.checked,
  )
        ...
        smsTemplates:
          smsTemplateRows.length > 0
            ? smsTemplateRows.map((item, index) => ({
                ...
                content: readString(
                  item.passContent,
                  basePayload.smsTemplates[index]?.content || '模板内容以短信设置页中的配置为准',
                ),
              }))
...
function adaptPaymentWays(items: PaymentWayApiItem[] | undefined | null) {
...
function deriveChannelName(
  items: DepositChannelApiItem[] | undefined | null,
  fallback: string,
) {
  ...
  if (channelNames.some((name) => name.includes('携程'))) return '携程直连'
  if (channelNames.some((name) => name.includes('美团'))) return '美团直连'
  return fallback
}

function adaptRoomPasswordStrategies(
  baseStrategies: SmartHotelGlobalSettingChoice[],
  roomCategoryRows: RoomCategoryApiItem[],
) {
...
function readRoomCategoryTimePair(row: RoomCategoryApiItem) {
...
function adaptSmsSendChoices(
  baseChoices: SmartHotelGlobalSettingChoice[],
  templateRows: SmsTemplateApiItem[],
) {
  if (templateRows.length === 0) return baseChoices

  const passwordTemplate = templateRows.find((item) => readString(item.name, '').includes('获得密码'))
  if (!passwordTemplate) return baseChoices

  return baseChoices.map((item, index) =>
    index === 0
      ? {
          ...item,
...
function readBooleanConfig(
  configs: SystemConfigApiItem[] | undefined | null,
  configKey: string,
  fallback: boolean,
) {
...
function pickLatestMeta<T extends { traceId: string; timestamp: string }>(responses: T[]) {
...
function buildSyncLabel(timestamp: string) {
...
function validateFilters(filters: SmartHotelGlobalSettingFilters) {
  if (!filters.campId.trim()) {
    throw new Error('全局设置门店参数不正确。')
  }
}

function toMockState(value: string | null): SmartHotelGlobalSettingMockState {
