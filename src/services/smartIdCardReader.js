...
export function createDefaultSmartIdCardReaderFilters(searchParams = new URLSearchParams()) {
    return {
...
export async function fetchSmartIdCardReaderDashboard(filters, signal, providerName = getSmartIdCardReaderProviderName()) {
...
export function getSmartIdCardReaderProviderName() {
...
function buildMockEnvelope(filters) {
    ...
    const records = filters.mockState === 'empty' ? [] : filterRecords(baseRecords, filters.deviceStatus, filters.keyword);
    const setupStatus = filters.deviceStatus === 'pending' ? '待调试' : '已连接';
    return {
        code: 0,
        message: 'success',
        data: createDashboardPayload(records, filters, setupStatus),
        traceId: filters.mockState === 'empty'
...
function createDashboardPayload(records, filters, setupStatus) {
    ...
    return {
        ...
        records,
        quickLinks,
        emptyState: filters.mockState === 'empty'
            ? {
                title: '当前筛选条件下暂无读卡记录',
...
function adaptSmartIdCardReaderEnvelope(envelope, provider) {
    if (envelope.code !== 0) {
...
function filterRecords(records, deviceStatus, keyword) {
    const normalizedKeyword = keyword.trim();
    return records.filter((record) => {
        if (deviceStatus !== 'all' && record.deviceStatus !== deviceStatus)
            return false;
        if (!normalizedKeyword)
            return true;
        return (record.guestName.includes(normalizedKeyword) ||
...
function toDatePreset(value) {
    return value === '7d' || value === '30d' ? value : 'today';
}
function toDeviceStatus(value) {
