...
export class SmsSettingServiceError extends Error {
    provider;
    contract;
    constructor(message, provider, contract) {
        super(message);
...
export function createDefaultSmsSettingFilters(searchParams = new URLSearchParams()) {
    return {
        campId: searchParams.get('campId') || DEFAULT_CAMP_ID,
        mockState: normalizeMockState(searchParams.get('mockState')) ?? readMockState(),
    };
}
export async function fetchSmsSettingDashboard(filters, signal, provider = readProvider()) {
...
export function readSmsSettingDiagnostics() {
    ...
    return {
        ...
        sections: [
            ...
            },
            {
                ...
                iconLabel: '租',
                templates: [
                    createTemplate('rent-collection', '长租催收短信', '【路客云】尊敬的{房型名称}{房间名称}租客{姓名}您好！您本周期{缴费开始时间}至{缴费结束时间}房租费合计：{应收金额}。'),
                    createTemplate('rent-monthly', '每月租金提醒', '【路客云】您的长租订单{订单}本期账单{开始时间}至{结束时间}需要催收，租客姓名：{租客姓名}，应收金额：{应收金额}。'),
                ],
            },
            {
                id: 'mall-order',
            ...
            },
            {
                ...
                templates: [
                    createTemplate('smart-password', '获得密码（智能入住）', '【路客云】您入住的房间{房源名称}{房间号}，门锁密码：{密码}#；点击{小程序跳转短链接}查看入住指引。'),
                    createTemplate('smart-register', '实名登记（智能入住）', '【路客云】您预订的房间可智能入住，点击{小程序跳转短链接}完成实名登记并获取门锁密码。'),
                    createTemplate('smart-wechat-register', '智住发送入住登记短信(微信公众号)', '【路客云】请先完成入住登记，点击{公众号跳转短链接}提交实名信息，审核后可自助办理入住。'),
                    createTemplate('smart-wecom-register', '智住发送入住登记短信(企微)', '【路客云】请点击{企微跳转短链接}进入企微服务页，完成入住登记后即可继续获取入住指引。'),
                ],
            },
            {
                id: 'door-lock',
            ...
            },
            {
                ...
                iconLabel: '其',
                templates: [
                    createTemplate('wecom-add-friend', '企微批量加好友', '【路客云】回馈新老用户，不定期推出优惠活动，点击{小程序跳转短链接}添加企微获取最新优惠信息。'),
                ],
            },
        ],
        rechargePlans: [
...
function createTemplate(id, title, content, enabled = true) {
    return {
        id,
        title,
        content,
...
function createContractSnapshot(provider, state, traceId, requestBody) {
    return {
        provider,
        state,
        traceId,
...
function writeDiagnostics(contract) {
    if (typeof window === 'undefined')
        return;
