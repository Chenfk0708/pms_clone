...
export const statisticsReportPresetOptions = [
...
const monthRoomTypeChannelTrend = [
...
export class StatisticsReportServiceError extends Error {
    ...
    request;
    response;
    constructor(message, provider, request, response) {
        super(message);
...
export function createDefaultStatisticsReportQuery() {
...
export function buildStatisticsReportQueryForPreset(preset, current) {
...
function buildStatisticsPresetRange(preset, provider) {
    if (provider === 'mock') {
        if (preset === 'yesterday')
            return { startDate: '2026-05-18', endDate: '2026-05-18', predictStartDate: '', predictEndDate: '' };
        if (preset === 'today')
            return { startDate: '2026-05-19', endDate: '2026-05-19', predictStartDate: '', predictEndDate: '' };
        if (preset === 'lastWeek')
            return { startDate: '2026-05-12', endDate: '2026-05-18', predictStartDate: '', predictEndDate: '' };
        if (preset === 'thisWeek')
            return { startDate: '2026-05-19', endDate: '2026-05-25', predictStartDate: '', predictEndDate: '' };
        if (preset === 'lastMonth')
            return { startDate: '2026-04-01', endDate: '2026-04-30', predictStartDate: '', predictEndDate: '' };
...
function resolveStatisticsReportCampId() {
    const envCampId = import.meta.env.VITE_PMS_CAMP_ID;
...
export function resolveStatisticsReportProvider() {
    ...
    if (value === 'mock')
        return 'mock';
    const envProvider = import.meta.env.VITE_STATISTICS_REPORT_PROVIDER;
    if (envProvider === 'api' || envProvider === 'real')
        return 'api';
    if (envProvider === 'mock')
        return 'mock';
    return 'api';
...
export function createStatisticsReportRequestBody(query) {
    const requestBody = {
        campId: query.campId,
        startDate: query.startDate,
        endDate: query.endDate,
    };
    if (query.predictStartDate)
        requestBody.predictStartDate = query.predictStartDate;
    if (query.predictEndDate)
        requestBody.predictEndDate = query.predictEndDate;
    if (query.roomCategoryIds.length > 0)
        requestBody.roomCategoryIds = query.roomCategoryIds;
...
export async function fetchStatisticsReportDashboard(input, signal) {
    ...
    if (state === 'error') {
        throw new StatisticsReportServiceError(`统计概览加载失败：${STATISTICS_REPORT_ENDPOINT} 返回业务错误`, provider, request, createNullEnvelope(503, 'statistics report mock failed', 'error'));
    }
...
async function fetchStatisticsReportApi(request, requestBody, signal) {
    let response;
    try {
        ...
        if (error instanceof DOMException && error.name === 'AbortError')
            throw error;
        throw new StatisticsReportServiceError(error instanceof Error ? error.message : '统计概览接口请求失败', 'api', request, createNullEnvelope(503, 'statistics report api request failed', 'api-request'));
    }
    if (!response.ok) {
        throw new StatisticsReportServiceError(`${STATISTICS_REPORT_ENDPOINT} 返回 HTTP ${response.status}`, 'api', request, createNullEnvelope(response.status, `statistics report api http ${response.status}`, 'api-http'));
    }
    ...
    if (success === false || (code !== null && code !== 0)) {
        ...
        throw new StatisticsReportServiceError(message, 'api', request, createNullEnvelope(code ?? 500, message, 'api-business'));
    }
    return {
        code: 0,
...
function adaptDashboard(provider, state, requestBody, envelope) {
    if (envelope.code !== 0) {
...
function createTrendMetrics(trend) {
...
function adaptSourceItems(list, total) {
...
function resolveMockResponse(request, requestBody) {
    ...
    throw new StatisticsReportServiceError('当前筛选条件暂不支持统计概览查询，请调整后重试', 'mock', normalizeQuery(createDefaultStatisticsReportQuery()), createNullEnvelope(422, `unsupported mock request: ${signature}`, 'unsupported'));
}
function mockSuffixForRequest(requestBody) {
    const signature = buildSignature(requestBody);
    if (signature === buildSignature(createStatisticsReportRequestBody(createDefaultStatisticsReportQuery())))
        return 'yesterday';
    if (signature === buildSignature(createStatisticsReportRequestBody(buildStatisticsReportQueryForPreset('today'))))
        return 'today';
    if (signature === buildSignature(createStatisticsReportRequestBody(buildStatisticsReportQueryForPreset('thisMonth'))))
        return 'this-month';
    if (signature ===
        buildSignature(createStatisticsReportRequestBody({
...
const mockResponseMap = {
    ...
    [buildSignature(createStatisticsReportRequestBody(buildStatisticsReportQueryForPreset('today')))]: todayResponse,
    [buildSignature(createStatisticsReportRequestBody(buildStatisticsReportQueryForPreset('thisMonth')))]: thisMonthResponse,
    [buildSignature(createStatisticsReportRequestBody({
        ...buildStatisticsReportQueryForPreset('thisMonth'),
        roomCategoryIds: ['1796425098965729282'],
        channelIds: [],
...
function normalizeRawStatisticsReport(value) {
    ...
    return rawResponse({
        ...
        allDayRoomFeePriceIncludingCommission: readNumber(record.allDayRoomFeePriceIncludingCommission) ?? readNumber(record.roomFeePriceIncludingCommission) ?? 0,
    });
}
function normalizeRawTrendPoint(value) {
    const record = asRecord(value);
...
function normalizeRawOrderSource(value) {
    const record = asRecord(value);
...
function isRawTrendPoint(value) {
    return value !== null;
}
function isRawOrderSource(value) {
    return value !== null;
}
function normalizeSummaryOcc(value) {
    return value > 0 && value <= 1 ? Number((value * 100).toFixed(2)) : value;
}
function normalizeTrendOcc(value) {
...
function isUnfilteredRangeQuery(request) {
    return (request.roomCategoryIds.length === 0 &&
        request.channelIds.length === 0 &&
        request.roomCategoryGroupIds.length === 0);
}
function createRangeResponse(request) {
...
function sumTrendValues(list, key) {
...
function rawTrend(date, businessIncome, roomFeePriceIncludingCommission, otherOrderExpense, writeDownIncome, occ, adr, revPar, openRoomCount) {
    return {
        date,
        businessIncome,
        roomFeePriceIncludingCommission,
...
function series(key, label, color, values) {
    return { key, label, color, values };
}
function buildSignature(requestBody) {
    const ordered = {};
