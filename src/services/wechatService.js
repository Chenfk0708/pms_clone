...
export async function fetchWechatServiceDashboard(query, signal) {
...
export function createWechatServiceExportTask(query) {
    const request = buildWechatServiceRequest(query);
    const diagnostics = {
...
export function resolveWechatServiceRuntimeConfig(location) {
...
export function resolveWechatServiceProvider() {
...
function readWechatServiceMockState() {
    const configured = readRuntimeConfig('pms.wechatServiceMockState') || import.meta.env.VITE_WECHAT_SERVICE_MOCK_STATE;
    if (configured === 'empty' || configured === 'error')
        return configured;
    return 'success';
}
async function fetchMockWechatService(query, _request, signal) {
...
async function fetchApiWechatService(query, request, signal) {
...
function buildWechatServiceRequest(query) {
    return {
        campId: query.campId,
...
function createBackendData(conversations, query) {
    return {
...
function createSummary(conversations) {
...
function createTodos(activeCount) {
    return [
...
function filterConversations(rows, query) {
...
function adaptConversation(conversation) {
