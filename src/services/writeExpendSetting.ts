...
export type WriteExpendSettingProviderName = 'mock' | 'api'
export type WriteExpendSettingMockState = 'success' | 'empty' | 'error'
export type WriteExpendSettingAction = 'list' | 'create'
export type WriteExpendSettingTab = 'income' | 'expense'

export type WriteExpendSettingItem = {
  id: string
  name: string
  groupType: number
  groupName: string
...
export type WriteExpendSettingGroup = {
  id: string
  groupType: number
  name: string
  items: WriteExpendSettingItem[]
}

export type WriteExpendSettingDiagnostics = {
  provider: WriteExpendSettingProviderName
  action: WriteExpendSettingAction
  state: WriteExpendSettingMockState
  endpoint: string
...
export type WriteExpendSettingPageData = {
  provider: WriteExpendSettingProviderName
  action: WriteExpendSettingAction
  state: WriteExpendSettingMockState
  endpoint: string
  requestBody: Record<string, unknown>
  tabs: Record<
    WriteExpendSettingTab,
    {
      totalItems: number
      groups: WriteExpendSettingGroup[]
    }
  >
  disabledItems: WriteExpendSettingItem[]
  ...
  traceId: string
  timestamp: string
  diagnostics: WriteExpendSettingDiagnostics
}
...
type WriteExpendSettingPayload = {
  paymentGroups: PaymentGroupDTO[]
  paymentWays: Array<{
    paymentWayId: string
...
const incomeItemNamesByGroup: Record<number, string[]> = {
  1: [
    '房费',
    '清洁费',
    '押金',
...
const expenseItemNamesByGroup: Record<number, string[]> = {
...
export function resolveWriteExpendSettingQuery(search: string): {
  mockState?: WriteExpendSettingMockState
  tab?: WriteExpendSettingTab
  mockDelayMs?: number
} {
...
export async function createWriteExpendSettingItem(input: {
  ...
  const duplicate = createMockPaymentGroups(storedItems)
    ...
    .some(
      (item) =>
        normalizeName(item.paymentTypeName) === normalizedName &&
  ...
  const nextCustomItem: StoredCustomItem = {
...
function buildPayloadFromPageData(pageData: WriteExpendSettingPageData): WriteExpendSettingPayload {
  return {
...
async function fetchWriteExpendSettingPageDataFromApi(requestBody: {
  ...
  const payload = (await response.json()) as HudsonEnvelope<{
    paymentGroups?: PaymentGroupDTO[]
  }>
...
async function fetchMockWriteExpendSettingEnvelope(
  state: WriteExpendSettingMockState,
  delayMs: number,
): Promise<UnifiedEnvelope<WriteExpendSettingPayload>> {
  await delay(delayMs)
...
function createMockPaymentGroups(customItems: StoredCustomItem[]): PaymentGroupDTO[] {
  return groupCatalog.map((group) => {
    const incomeItems = (incomeItemNamesByGroup[group.groupType] ?? []).map((name, index) =>
      createPaymentType({
    ...
    const expenseItems = (expenseItemNamesByGroup[group.groupType] ?? []).map((name, index) =>
      createPaymentType({
...
function adaptEnvelope(
  ...
  state: WriteExpendSettingMockState,
  requestBody: Record<string, unknown>,
  envelope: UnifiedEnvelope<WriteExpendSettingPayload>,
...
function buildPageData(input: {
  ...
  endpoint: string
  requestBody: Record<string, unknown>
  payload: WriteExpendSettingPayload
  traceId: string
  timestamp: string
}): WriteExpendSettingPageData {
  ...
  for (const paymentGroup of input.payload.paymentGroups) {
    for (const paymentType of paymentGroup.paymentTypes) {
      const nextItem: WriteExpendSettingItem = {
  ...
  } satisfies WriteExpendSettingDiagnostics
...
function persistDiagnostics(diagnostics: WriteExpendSettingDiagnostics) {
  if (typeof window === 'undefined') return
  window.localStorage.setItem(WRITE_EXPEND_SETTING_LAST_REQUEST_KEY, JSON.stringify(diagnostics))
}

function readStoredCustomItems(): StoredCustomItem[] {
  if (typeof window === 'undefined') return []
  ...
  try {
    const parsed = JSON.parse(rawText) as StoredCustomItem[]
...
function writeStoredCustomItems(items: StoredCustomItem[]) {
  if (typeof window === 'undefined') return
  window.localStorage.setItem(WRITE_EXPEND_SETTING_CUSTOM_ITEMS_KEY, JSON.stringify(items))
}

function resolveWriteExpendSettingMockState(): WriteExpendSettingMockState {
