...
export function validateOptionalMainlandMobile(value: string) {
  const normalized = value.trim()
...
export function validateRequiredMainlandMobile(value: string) {
  const normalized = value.trim()
  if (!normalized || !mainlandMobilePattern.test(normalized)) {
    return inputValidationMessages.mobile
  }
...
export function validateOptionalContactPhone(value: string) {
  const normalized = value.trim()
...
export function validateCredentialNumber(credentialType: string, credentialNumber: string) {
...
function isValidResidentIdCard(value: string) {
