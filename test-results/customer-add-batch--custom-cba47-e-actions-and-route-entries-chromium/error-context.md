...
- Name: customer-add-batch.spec.ts >> /customer/addBatch gives feedback for visible actions and route entries
- Location: tests\customer-add-batch.spec.ts:39:1

# Error details

```
Error: locator.click: Error: strict mode violation: getByLabel('批量加好友核心指标').getByRole('button', { name: /预计可加好友/ }) resolved to 2 elements:
    1) <button type="button" class="customer-add-batch-metric">…</button> aka getByRole('button', { name: '预计可加好友 4人 近 30 天有订单且未添加企微的客户' })
    2) <button type="button" class="customer-add-batch-metric">…</button> aka getByRole('button', { name: '转化率 41.3% 已添加人数 / 预计可加好友人数' })

Call log:
  - waiting for getByLabel('批量加好友核心指标').getByRole('button', { name: /预计可加好友/ })
      ...
      - generic [ref=e132]:
        ...
        - region "批量加好友筛选" [ref=e142]:
          ...
          - generic [ref=e158]:
            ...
            - button "导 出" [active] [ref=e162] [cursor=pointer]
        - status "批量加好友操作反馈" [ref=e163]: 已生成批量加好友导出任务
