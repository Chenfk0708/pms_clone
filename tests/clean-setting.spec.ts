  ...
  await page.route('**/api/select/poi/page/get', async (route) => {
    await route.fulfill({
      ...
      body: JSON.stringify({
        ...
        data: {
          list: [
            { poiId: 'all', poiName: '全部门店' },
            { poiId: 'qianhai', poiName: '前海店' },
          ],
        },
      }),
    })
...
test('/cleanManage/cleanSetting supports filters refresh export details and route handoff', async ({ page }) => {
  ...
  await page.getByRole('option', { name: '前海店' }).click()
  ...
  await expect(page.getByRole('region', { name: '当前筛选条件' })).toContainText('2026-05-20')
