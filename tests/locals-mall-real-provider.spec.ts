...
test('/version/localsMall api provider adapts overview and detail contracts', async ({ page }) => {
  const requests: Array<{ path: string; body: Record<string, unknown> }> = []

  await page.setViewportSize({ width: 1440, height: 900 })
  await page.addInitScript(() => {
    window.localStorage.setItem('pms_token', 'locals-mall-contract-token')
    window.localStorage.setItem('pms_user', JSON.stringify({ id: '12001', name: '演示管理员', campName: '演示门店' }))
    window.localStorage.setItem('pmsCampId', '10001')
    window.localStorage.setItem('pms.localsMall.provider', 'api')
  })
  ...
  await page.route('**/api/weiRoomCategories/page/get', async (route) => {
    ...
    await route.fulfill({
      json: {
        code: 0,
        message: 'success',
        traceId: 'locals-mall-overview-001',
        timestamp: '2026-06-01T14:35:00+08:00',
        data: {
          list: [
            {
            ...
            {
              channelRoomCategoryId: '99602',
              channelRoomCategoryName: '智能门锁',
              description: '门锁硬件统一采购入口',
              mainPhoto: 'https://example.com/smart-lock.png',
              goodsType: 8,
              lowestSellingPrice: 99800,
            },
          ],
        },
      },
  ...
  await page.route('**/api/youzan/commodity/get', async (route) => {
    ...
    await route.fulfill({
      json: {
        code: 0,
        message: 'success',
        traceId: 'locals-mall-commodity-001',
        timestamp: '2026-06-01T14:36:00+08:00',
        data: {
          commodityId: '99601',
  ...
  await page.route('**/api/rooms/get', async (route) => {
    ...
    await route.fulfill({
      json: {
        code: 0,
        message: 'success',
        traceId: 'locals-mall-rooms-001',
        timestamp: '2026-06-01T14:36:30+08:00',
        data: {
          roomCategoryRooms: [
  ...
  await page.route('**/api/paymentTypes/get/v2', async (route) => {
    ...
    await route.fulfill({
      json: {
        code: 0,
        message: 'success',
        traceId: 'locals-mall-payment-001',
        timestamp: '2026-06-01T14:36:40+08:00',
        data: {
          paymentGroups: [
            {
              groupType: 1,
  ...
  await expect(page.getByRole('heading', { name: '门卡管理系统' })).toBeVisible({ timeout: 15_000 })
  await expect(page.getByRole('heading', { name: '智能门锁' })).toBeVisible({ timeout: 15_000 })
  await expect(contract).toContainText('"commodityId": "99601"')
  ...
  await expect(page.getByText('3 间房')).toBeVisible({ timeout: 15_000 })
