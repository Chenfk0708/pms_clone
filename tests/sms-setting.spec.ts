...
test('/setting/balanceAndTemplate 通过服务契约渲染短信设置成功态', async ({ page }) => {
  ...
  await expect(page.getByText('剩余短信')).toBeVisible()
  ...
  await expect(page.getByText('启用渠道')).toBeVisible()
  await expect(page.getByText('阿里云短信服务')).toBeVisible()
  await expect(page.getByText('签名')).toBeVisible()
  await expect(page.getByText('【路客云】', { exact: true })).toBeVisible()

  const sections = page.locator('.sms-template-card')
  ...
  await expect(page.getByText('门锁临时密码')).toBeVisible()
...
test('/setting/balanceAndTemplate 支持充值、充值记录、渠道签名修改和去设置跳转', async ({ page }) => {
  await page.goto(appUrl(pagePath), { waitUntil: 'domcontentloaded' })
  ...
  await expect(rechargeDialog.getByRole('button', { name: '500条', exact: true })).toBeVisible()
  await expect(rechargeDialog.getByRole('button', { name: '1000条', exact: true })).toBeVisible()
  await expect(rechargeDialog.getByRole('button', { name: '2000条', exact: true })).toBeVisible()
  await expect(rechargeDialog.getByRole('button', { name: '5000条', exact: true })).toBeVisible()
  await expect(rechargeDialog.getByRole('button', { name: '15000条', exact: true })).toBeVisible()
  await rechargeDialog.getByRole('button', { name: '5000条', exact: true }).click()
  await expect(page.getByRole('status', { name: '短信设置操作反馈' })).toContainText('已选择 5000 条短信套餐')
  ...
  await page.locator('.sms-setting-meta > div').first().getByRole('button', { name: '修改' }).click()
  const channelDialog = page.getByRole('dialog', { name: '启用渠道' })
  await expect(channelDialog).toContainText('阿里云短信服务')
  ...
  await page.locator('.sms-setting-meta > div').nth(1).getByRole('button', { name: '修改' }).click()
