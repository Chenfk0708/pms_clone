...
test('/setting/sortSetting shows the captured default sort-setting shell', async ({ page }) => {
  ...
  await page.goto(appUrl('/setting/sortSetting?provider=mock'))
...
test('/setting/sortSetting supports tabs, reorder feedback, and runtime states', async ({ page }) => {
  ...
  await expect(page.getByTestId('sort-setting-service-contract')).toContainText('/channelRoomCategories/seqs')

  await page.goto(appUrl('/setting/sortSetting?provider=mock&mockState=empty&tab=room'))
  await expect(page.getByRole('tab', { name: '房型排序' })).toHaveAttribute('aria-selected', 'true')
  ...
  await page.goto(appUrl('/setting/sortSetting?provider=mock&mockState=error'))
...
async function installMockSession(page: import('@playwright/test').Page) {
  await page.addInitScript(() => {
    window.localStorage.setItem('pms_token', 'sort-setting-mock-token')
